...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v7';
  ExpectedSource =
    'tests/parity/legacy-oracle/v7/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v7/build-recipe.json';
  ExpectedScenario = 'audio.realistic-hiss-noise-floor';
  ExpectedSampleRate = 11025;
  ExpectedBlockSize = 512;
  ExpectedSeed = 12345;
  ExpectedBandwidthHz = 500;
  ExpectedPitchHz = 600;
  ExpectedTotalBlocks = 12;
...
begin
  ...
  Values.Add(
    ...
    + '|total-blocks=' + IntToStr(TotalBlocks)
  ...
  CreateRuntime(Seed);
  try
    for BlockIndex := 0 to StartupRequestCount - 1 do
    begin
      BlockData := Tst.GetAudio;
      if (Length(BlockData) <> 1)
        or (BlockData[0] <> 0)
        or (Tst.BlockNumber <> BlockIndex + 1) then
        raise Exception.Create(
          'CE startup request framing changed');
    end;

    RandSeed := Seed;
    SetLength(Aggregate, TotalBlocks * BlockSize);
    Peak := 0;
    Rms := 0;
    for BlockIndex := 0 to TotalBlocks - 1 do
    begin
      BlockData := Tst.GetAudio;
      if (Length(BlockData) <> BlockSize)
        or (Tst.BlockNumber
            <> StartupRequestCount + BlockIndex + 1)
        or (Tst.Stations.Count <> 0)
        or (Tst.Me.State = stSending)
        or MainForm.HandleAllocated
        or MainForm.Panel2.HandleAllocated
      ...
      SetLength(NormalizedBlock, BlockSize);
      ...
      begin
        ...
        Aggregate[BlockIndex * BlockSize + SampleIndex] := Sample;
  ...
  if Values.Count <> 14 then
    raise Exception.Create(
      'realistic hiss scenario emitted an invalid row count');
end;
...
begin
  ...
  try
    ...
    ObserveNoiseFloor(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
