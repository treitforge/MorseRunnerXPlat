...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v5';
  ExpectedSource =
    'tests/parity/legacy-oracle/v5/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v5/build-recipe.json';
  ExpectedScenario =
    'ux.enter-esm-partial-call-message-selection';
  ExpectedContestId = 'scWpx';
  ExpectedRunModeId = 'rmPileup';
...
begin
  ...
  Ini.SerialNR := snStartContest;
  Ini.SimContest := scWpx;
  Ini.ActiveContest := @Ini.ContestDefinitions[scWpx];
  Ini.UserExchangeTbl[scWpx] := '5NN #';
  Ini.UserExchange1[scWpx] := '5NN';
  Ini.UserExchange2[scWpx] := '#';
end;
...
begin
  ...
  if (Tst.ClassType <> TCqWpx)
    or (Tst.Me.ClassType <> TMyStation)
    or (Tst.Me.MyCall <> ExpectedStationCall)
    or (Tst.Me.SentExchTypes.Exch1 <> etRST)
    or (Tst.Me.SentExchTypes.Exch2 <> etSerialNr)
    or (MainForm.RecvExchTypes.Exch1 <> etRST)
    or (MainForm.RecvExchTypes.Exch2 <> etSerialNr)
    or (Tst.Me.State <> stListening)
    or (Tst.Stations.Count <> 0) then
    raise Exception.Create('CE CQ WPX runtime is not configured');
  RequireNoHandles;
end;
...
begin
  ...
  if RequireString(Input, 'contestId') <> ExpectedContestId then
    raise Exception.Create('contestId does not match the fixed vector');
  if RequireString(Input, 'runModeId') <> ExpectedRunModeId then
  ...
  Actions := RequireActions(Input);

  Values.Add(
    'configuration'
    + '|scenario=' + ExpectedScenario
    + '|contest=' + ExpectedContestId
    + '|run-mode=' + ExpectedRunModeId
    + '|station=' + ExpectedStationCall
    + '|seed=' + IntToStr(ExpectedSeed)
    + '|action-count=' + IntToStr(ExpectedActionCount));

  for Index := 0 to Actions.Count - 1 do
  begin
    Action := TJSONObject(Actions.Items[Index]);
    if RequireBoolean(Action, 'reset') then
      CreateRuntime
    else if not Assigned(Tst)
      or not Assigned(Keyer)
      or not Assigned(gDXCCList)
      or not Assigned(MainForm) then
      raise Exception.Create(
        'an action requested continuation without a CE runtime');

    ObserveAction(Values, Index, Action);

    if Index = Actions.Count - 1 then
      NextReset := True
    else
      NextReset :=
        RequireBoolean(
          TJSONObject(Actions.Items[Index + 1]),
          'reset');
    if NextReset then
      DestroyRuntime
    else
    begin
      ...
      if Tst.Me.State <> stListening then
        raise Exception.Create(
          'CE did not finish the transmission before correction');
      if Tst.Stations.Count <> 0 then
        raise Exception.Create(
          'CE created a remote station before correction');
    end;
  end;

  if Values.Count <> ExpectedActionCount + 1 then
    raise Exception.Create('Enter/ESM scenario emitted an invalid row count');
end;
