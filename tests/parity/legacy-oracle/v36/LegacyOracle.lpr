...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v36';
  ExpectedSource =
    'tests/parity/legacy-oracle/v36/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v36/build-recipe.json';
  ExpectedScenario =
...
procedure ObserveCqwwRandomConsumptionContest(
  const Values: TStrings;
...
procedure ObserveCqwwRandomConsumptionRemoteExchangeFormat(
  const Values: TStrings;
  const Input: TJSONObject);
begin
  RequireExactObjectFields(
  ...
  ObserveCqwwRandomConsumptionContest(
    Values,
    Ord(scCQWW),
    'scCQWW',
    ExpectedRemoteCall,
  ...
  if Values.Count <> 3 then
    raise Exception.Create(
      'CQ WW random-consumption scenario emitted invalid rows');
end;
...
begin
  ...
  try
    ...
    ObserveCqwwRandomConsumptionRemoteExchangeFormat(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
