...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v28';
  ExpectedSource =
    'tests/parity/legacy-oracle/v28/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v28/build-recipe.json';
  ExpectedScenario =
    'contest.sst-remote-exchange-format-seed-12345';
  ExpectedStationCall = 'W7SST';
  ExpectedRemoteCall = 'K1ABC';
  ExpectedRst = 599;
  ExpectedExchange1 = 'BRUCE';
  ExpectedExchange2 = 'MA';
  ExpectedSeed = 12345;
...
procedure ObserveSstRemoteExchangeFormat(
  const Values: TStrings;
  const Input: TJSONObject);
var
...
begin
  ...
  if RequireString(Input, 'contestId') <> 'scSst' then
    raise Exception.Create('contestId does not match the fixed vector');
  ...
  if Values.Count <> 2 then
    raise Exception.Create('SST exchange scenario emitted invalid rows');
end;
...
begin
  ...
  try
    ...
    ObserveSstRemoteExchangeFormat(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
