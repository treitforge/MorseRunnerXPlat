...
{$ifndef V52_TUI_HST_WPM_PAGE_UP_ROUNDING}
  {$fatal V52_TUI_HST_WPM_PAGE_UP_ROUNDING must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v52';
  ExpectedSource =
    'tests/parity/legacy-oracle/v52/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v52/build-recipe.json';
  ExpectedScenario =
...
procedure ObserveHstWpmPageUpRounding(
  const Values: TStrings;
  const Input: TJSONObject);
var
...
begin
  ...
  if (Seed <> ExpectedSeed)
    or (InitialWpm <> 32)
    or (ExpectedAfterWpm <> 35)
    or (RunModeId <> 'rmHst') then
    raise Exception.Create(
      'HST WPM PageUp input does not match its pinned contract');

  CreateRuntime(Seed, False);
  try
    Ini.RunMode := rmHst;
    MainForm.SetWpm(InitialWpm);
    if Ini.Wpm <> InitialWpm then
      raise Exception.Create(
        'CE HST WPM PageUp runtime did not start at its contract');
    Values.Add(
      'configuration'
      + '|run-mode=' + RunModeId
      + '|seed=' + IntToStr(Seed)
    ...
    if (Key <> 0) or (Ini.Wpm <> ExpectedAfterWpm) then
      raise Exception.Create(
        'CE HST WPM PageUp handler reached an invalid value');
    Values.Add(
      'wpm-after-hst-page-up'
      + '|wpm=' + IntToStr(Ini.Wpm)
      + '|delta-wpm=' + IntToStr(Ini.Wpm - InitialWpm));
    if Values.Count <> 3 then
      raise Exception.Create(
        'HST WPM PageUp capture emitted an invalid row count');
  finally
    DestroyRuntime;
  end;
end;
...
begin
  ...
  try
    ...
    ObserveHstWpmPageUpRounding(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
