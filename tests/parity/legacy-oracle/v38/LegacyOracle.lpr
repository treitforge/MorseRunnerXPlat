...
{$ifndef V38_OPERATOR_MONITOR_MUTE}
  {$fatal V38_OPERATOR_MONITOR_MUTE must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v38';
  ExpectedSource =
    'tests/parity/legacy-oracle/v38/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v38/build-recipe.json';
  ExpectedScenario =
  ...
  ExpectedMutedMonitorLevelDb = -60;
  ExpectedTerminalOrdinal = 1024;
  ExpectedTerminalBits = '3e320354';
  ExpectedQskEnvelopeSamples = 50176;
  ExpectedMonitorFirstDivergence = 310;
  ExpectedFullMonitorHash =
    '7d925cbba9a0bb2e86a48c5a1777c347cfed68080a559446d8e3ed3c9d6af4ee';
  ExpectedMutedMonitorHash =
...
function ObserveMonitorRun(
  const Seed: Integer;
  const MonitorLevelDb: Integer): TMonitorRunObservation;
var
  BlockData: TSingleArray;
  SampleIndex: Integer;
  StartupIndex: Integer;
begin
  ...
  try
    Ini.Qsk := False;
    MainForm.VolumeSlider1.Db := MonitorLevelDb;
...
var
  ...
  MutedMonitorLevelDb: Integer;
  PitchHz: Integer;
  ProbeIndexes: TIntegerArray;
  ProbeText: string;
  FullMonitor: TMonitorRunObservation;
  MutedMonitor: TMonitorRunObservation;
  SampleRate: Integer;
  Seed: Integer;
  StationCall: string;
  StartupRequestCount: Integer;
...
begin
  ...
  FullMonitorLevelDb := RequireInteger(
    ...
    -60,
    0);
  MutedMonitorLevelDb := RequireInteger(
    Input,
    'mutedMonitorLevelDb',
    -60,
    0);
  ...
  if (SampleRate <> ExpectedSampleRate)
    ...
    or (MutedMonitorLevelDb <> ExpectedMutedMonitorLevelDb)
  ...
  FullMonitor := ObserveMonitorRun(Seed, FullMonitorLevelDb);
  MutedMonitor := ObserveMonitorRun(Seed, MutedMonitorLevelDb);
  if (FullMonitor.MessageText <> ExpectedLocalMessage)
    or (MutedMonitor.MessageText <> ExpectedLocalMessage)
    or (FullMonitor.EnvelopeSamples <> ExpectedQskEnvelopeSamples)
    or (FullMonitor.EnvelopeSamples <> MutedMonitor.EnvelopeSamples)
    or (FullMonitor.SendPosition <> ExpectedBlockSize)
    or (MutedMonitor.SendPosition <> ExpectedBlockSize)
    or (SingleBits(FullMonitor.TerminalRandom) <> ExpectedTerminalBits)
    or (SingleBits(MutedMonitor.TerminalRandom) <> ExpectedTerminalBits) then
    raise Exception.Create('CE monitor fixed observation changed');

  Divergence := FirstDivergence(FullMonitor.Block, MutedMonitor.Block);
  if (Divergence <> ExpectedMonitorFirstDivergence)
    or (FloatBlockSha256(FullMonitor.Block) <> ExpectedFullMonitorHash)
    or (FloatBlockSha256(MutedMonitor.Block) <> ExpectedMutedMonitorHash)
    or (ProbeBits(FullMonitor.Block, ProbeIndexes)
        <> ExpectedFullMonitorProbeBits)
    or (ProbeBits(MutedMonitor.Block, ProbeIndexes)
        <> ExpectedMutedMonitorProbeBits) then
  ...
  Values.Add(
    ...
    + '|pitch-hz=' + IntToStr(PitchHz)
    + '|station-call=' + StationCall
    + '|monitor-levels-db=' + IntToStr(FullMonitorLevelDb)
    + ',' + IntToStr(MutedMonitorLevelDb)
  ...
  Values.Add(
    ...
    + '|exact-equal=false'
    + '|first-divergence=' + IntToStr(Divergence)
    + '|monitor-full-float-sha256=' + FloatBlockSha256(FullMonitor.Block)
    + '|monitor-muted-float-sha256=' + FloatBlockSha256(MutedMonitor.Block));
  Values.Add(
    'terminal-random'
    + '|ordinal=' + IntToStr(TerminalRandomOrdinal)
    + '|monitor-full-single-bits=' + SingleBits(FullMonitor.TerminalRandom)
    + '|monitor-muted-single-bits=' + SingleBits(MutedMonitor.TerminalRandom));
