...
{$ifndef V42_RUNTIME_RIT_CHANGE}
  {$fatal V42_RUNTIME_RIT_CHANGE must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v42';
  ExpectedSource =
    'tests/parity/legacy-oracle/v42/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v42/build-recipe.json';
  ExpectedScenario =
  ...
  ExpectedRuntimeRitFirstDivergence = 152;
  ...
  ExpectedFixedRitSecondProbeBits =
    '3db633f1,3debccf4,3dda374c,3dd939d5,3e00574a,3de129f6,'
    + 'bd9810c4,bbd51ede,3c49049f,3ba55221,3d8014b9,3dca8849';
  ExpectedRuntimeRitSecondProbeBits =
    '3db633f1,3debccf4,3dda374c,3dd939d5,3e00574a,3de129f6,'
    + 'bd958b5a,bbcfe4d9,be1d2466,3e1302b9,3e09c1fd,3db2f8eb';
...
function ObserveRitRun(
  const Seed: Integer;
  const RuntimeRitAfterFirstBlock: Boolean): TMonitorRunObservation;
var
  BlockIndex: Integer;
  BlockData: TSingleArray;
  SampleIndex: Integer;
  ...
  try
    ...
    begin
      ...
      if (Length(BlockData) <> 1)
        ...
        or (Tst.Stations.Count <> 0) then
        raise Exception.Create(
          'CE runtime-RIT startup framing changed');
    end;
    ...
    if (Tst.Stations.Count <> 1)
      or (Ini.Rit <> 0) then
      raise Exception.Create(
        'CE runtime-RIT fixed station did not start');

    for BlockIndex := 0 to 1 do
    begin
      if RuntimeRitAfterFirstBlock and (BlockIndex = 1) then
      begin
        MainForm.Panel8MouseDown(
          ...
          0);
        if Ini.Rit <> ExpectedRuntimeRitHz then
          raise Exception.Create(
            'CE runtime-RIT handler did not reach +50 Hz');
      end;
      ...
      if (Length(BlockData) <> ExpectedBlockSize)
        ...
        or (Ini.Rit
            <> IfThen(
              RuntimeRitAfterFirstBlock and (BlockIndex = 1),
              ExpectedRuntimeRitHz,
              0)) then
        raise Exception.Create(
          'CE runtime-RIT capture left its pinned path');
...
begin
  ...
  if (SampleRate <> ExpectedSampleRate)
    ...
    raise Exception.Create(
      'runtime-RIT input does not match its pinned contract');
  ...
  if Length(ProbeIndexes) <> ExpectedProbeCount then
    raise Exception.Create(
      'runtime-RIT probe count changed');
  ProbeText := '';
  for Index := 0 to ExpectedProbeCount - 1 do
  begin
    if ProbeIndexes[Index] <> ExpectedProbeSampleIndexes[Index] then
      raise Exception.Create(
        'runtime-RIT probes do not match their pinned contract');
    if Index > 0 then
      ProbeText := ProbeText + ',';
    ProbeText := ProbeText + IntToStr(ProbeIndexes[Index]);
  end;

  FixedRit := ObserveRitRun(Seed, False);
  RuntimeRit := ObserveRitRun(Seed, True);
  if not CompareMem(
      @FixedRit.Blocks[0][0],
      @RuntimeRit.Blocks[0][0],
      ExpectedBlockSize * SizeOf(Single)) then
    raise Exception.Create(
      'CE runtime-RIT first block changed before mutation');
  Divergence := FirstDivergence(
    FixedRit.Blocks[1],
    RuntimeRit.Blocks[1]);
  if (Divergence <> ExpectedRuntimeRitFirstDivergence)
    ...
    raise Exception.Create(
      'CE runtime-RIT fixed observation changed');
  ...
  Values.Add(
    ...
    + '|absolute-blocks=6,7'
    + '|rit-change-hz=0-to-' + IntToStr(ExpectedRuntimeRitHz)
    + '|change-before-absolute-block=7'
  ...
  if Values.Count <> ExpectedRuntimeRitValueCount then
    raise Exception.Create(
      'runtime-RIT capture emitted an invalid row count');
end;
...
begin
  ...
  try
    ...
    ObserveRuntimeRitChange(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
