...
{$ifndef V39_OPERATOR_MONITOR_RUNTIME_CHANGE}
  {$fatal V39_OPERATOR_MONITOR_RUNTIME_CHANGE must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v39';
  ExpectedSource =
    'tests/parity/legacy-oracle/v39/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v39/build-recipe.json';
  ExpectedScenario =
  ...
  ExpectedRuntimeMutedSecondMonitorHash =
    '4d62f0d47d5d84552b1e30ae49c93f6ac69c5ebb9edad4cd655bfa5eec01e3a2';
  ExpectedFullMonitorProbeBits =
  ...
  ExpectedRuntimeMutedSecondMonitorProbeBits =
    '3eb3ed67,3f007b40,3f1780a0,3f0e26cc,3eddd0a4,3e8492a0,'
    + '3f1c4001,3f0e1c03,b39e2a60,b41f20fe,b3bfb1c7,b29f663d';
...
function ObserveMonitorRun(
  const Seed: Integer;
  const RuntimeMuteAfterFirstBlock: Boolean): TMonitorRunObservation;
var
  BlockIndex: Integer;
  BlockData: TSingleArray;
  SampleIndex: Integer;
  ...
  try
    ...
    begin
      ...
      if (Length(BlockData) <> 1)
        ...
        raise Exception.Create('CE monitor startup framing changed');
    end;
    ...
    if (Tst.Me.State <> stSending)
      ...
      raise Exception.Create('CE monitor local CQ did not start');

    Result.MessageText := Tst.Me.MsgText;
    Result.EnvelopeSamples := Length(Tst.Me.Envelope);
    for BlockIndex := 0 to 1 do
    begin
      if RuntimeMuteAfterFirstBlock and (BlockIndex = 1) then
        MainForm.VolumeSlider1.Db := ExpectedRuntimeMonitorLevelDb;
      BlockData := Tst.GetAudio;
      if (Length(BlockData) <> ExpectedBlockSize)
        or (Tst.BlockNumber
        ...
        or (Round(MainForm.VolumeSlider1.Db)
            <> IfThen(
              RuntimeMuteAfterFirstBlock and (BlockIndex = 1),
              ExpectedRuntimeMonitorLevelDb,
              ExpectedMonitorLevelDb)) then
        raise Exception.Create('CE monitor capture left its pinned path');
...
procedure ObserveOperatorMonitorMute(
  const Values: TStrings;
  const Input: TJSONObject);
var
  BandwidthHz: Integer;
  ...
  Index: Integer;
  MessageText: string;
  FullMonitorLevelDb: Integer;
  RuntimeMonitorLevelDb: Integer;
  PitchHz: Integer;
  ProbeIndexes: TIntegerArray;
  ProbeText: string;
  FullMonitor: TMonitorRunObservation;
  RuntimeMutedMonitor: TMonitorRunObservation;
  SampleRate: Integer;
  Seed: Integer;
  StationCall: string;
  StartupRequestCount: Integer;
...
begin
  ...
  BandwidthHz := RequireInteger(Input, 'bandwidthHz', 1, MaxInt);
  PitchHz := RequireInteger(Input, 'pitchHz', 1, MaxInt);
  FullMonitorLevelDb := RequireInteger(
    Input,
    'fullMonitorLevelDb',
    -60,
    0);
  RuntimeMonitorLevelDb := RequireInteger(
    Input,
    'runtimeMonitorLevelDb',
    -60,
    0);
  ...
  if (SampleRate <> ExpectedSampleRate)
    ...
    or (BandwidthHz <> ExpectedBandwidthHz)
    or (PitchHz <> ExpectedPitchHz)
    or (FullMonitorLevelDb <> ExpectedMonitorLevelDb)
    or (RuntimeMonitorLevelDb <> ExpectedRuntimeMonitorLevelDb)
    or (StationCall <> ExpectedStationCall)
    or (MessageText <> ExpectedLocalMessage)
    or (Length(ProbeIndexes) <> ExpectedProbeCount) then
    raise Exception.Create(
      'monitor input does not match its pinned contract');
  for Index := 0 to ExpectedProbeCount - 1 do
    if ProbeIndexes[Index] <> ExpectedProbeSampleIndexes[Index] then
      raise Exception.Create(
        'monitor probes do not match their pinned contract');

  FullMonitor := ObserveMonitorRun(Seed, False);
  RuntimeMutedMonitor := ObserveMonitorRun(Seed, True);
  if (FullMonitor.MessageText <> ExpectedLocalMessage)
    or (RuntimeMutedMonitor.MessageText <> ExpectedLocalMessage)
    or (FullMonitor.EnvelopeSamples <> ExpectedQskEnvelopeSamples)
    or (FullMonitor.EnvelopeSamples
        <> RuntimeMutedMonitor.EnvelopeSamples)
    or (FullMonitor.SendPosition <> ExpectedBlockSize * 2)
    or (RuntimeMutedMonitor.SendPosition <> ExpectedBlockSize * 2)
    or (SingleBits(FullMonitor.TerminalRandom) <> ExpectedTerminalBits)
    or (SingleBits(RuntimeMutedMonitor.TerminalRandom)
        <> ExpectedTerminalBits) then
    raise Exception.Create('CE monitor fixed observation changed');

  if not CompareMem(
      @FullMonitor.Blocks[0][0],
      @RuntimeMutedMonitor.Blocks[0][0],
      ExpectedBlockSize * SizeOf(Single)) then
    raise Exception.Create('CE monitor first block changed before mutation');
  Divergence := FirstDivergence(
    FullMonitor.Blocks[1],
    RuntimeMutedMonitor.Blocks[1]);
  if (Divergence <> ExpectedMonitorFirstDivergence)
    or (FloatBlockSha256(FullMonitor.Blocks[0])
        <> ExpectedFirstMonitorHash)
    or (FloatBlockSha256(FullMonitor.Blocks[1])
        <> ExpectedFullSecondMonitorHash)
    or (FloatBlockSha256(RuntimeMutedMonitor.Blocks[1])
        <> ExpectedRuntimeMutedSecondMonitorHash)
    or (ProbeBits(FullMonitor.Blocks[0], ProbeIndexes)
        <> ExpectedFullMonitorProbeBits)
    or (ProbeBits(FullMonitor.Blocks[1], ProbeIndexes)
        <> ExpectedFullSecondMonitorProbeBits)
    or (ProbeBits(RuntimeMutedMonitor.Blocks[1], ProbeIndexes)
        <> ExpectedRuntimeMutedSecondMonitorProbeBits) then
    raise Exception.Create('CE monitor audio observation changed');
  ProbeText := '';
  for Index := 0 to High(ProbeIndexes) do
  begin
    if Index > 0 then
  ...
  Values.Add(
    ...
    + '|pitch-hz=' + IntToStr(PitchHz)
    + '|station-call=' + StationCall
    + '|monitor-change-db=' + IntToStr(FullMonitorLevelDb)
    + '-to-' + IntToStr(RuntimeMonitorLevelDb)
    + '|change-before-absolute-block=7'
    + '|qrn=false|qrm=false|qsb=false|flutter=false|qsk=false|lids=false');
  Values.Add(
    'local-message'
    + '|text=' + FullMonitor.MessageText
    + '|rendered-local-samples=' + IntToStr(FullMonitor.SendPosition)
    + '|probe-sample-indexes=' + ProbeText);
  AddBlockStatistics(
    'monitor-before-change-block[0]',
    FullMonitor.Blocks[0],
    ProbeIndexes,
    Values);
  AddBlockStatistics(
    'monitor-fixed-full-block[1]',
    FullMonitor.Blocks[1],
    ProbeIndexes,
    Values);
  AddBlockStatistics(
    'monitor-runtime-muted-block[1]',
    RuntimeMutedMonitor.Blocks[1],
    ProbeIndexes,
    Values);
  Values.Add(
    ...
    + '|exact-equal=false'
    + '|first-divergence=' + IntToStr(Divergence)
    + '|monitor-fixed-full-float-sha256='
    + FloatBlockSha256(FullMonitor.Blocks[1])
    + '|monitor-runtime-muted-float-sha256='
    + FloatBlockSha256(RuntimeMutedMonitor.Blocks[1]));
  Values.Add(
    'terminal-random'
    + '|ordinal=' + IntToStr(TerminalRandomOrdinal)
    + '|monitor-full-single-bits=' + SingleBits(FullMonitor.TerminalRandom)
    + '|monitor-runtime-muted-single-bits='
    + SingleBits(RuntimeMutedMonitor.TerminalRandom));

  if Values.Count <> ExpectedMonitorValueCount then
    raise Exception.Create('monitor capture emitted an invalid row count');
end;
...
begin
  ...
  try
    ...
    ObserveOperatorMonitorMute(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
