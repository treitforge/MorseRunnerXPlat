...
{$ifndef V40_RUNTIME_QSK_CHANGE}
  {$fatal V40_RUNTIME_QSK_CHANGE must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v40';
  ExpectedSource =
    'tests/parity/legacy-oracle/v40/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v40/build-recipe.json';
  ExpectedScenario =
  ...
  ExpectedRuntimeQskValueCount = 7;
  ...
  ExpectedRuntimeQskFirstDivergence = 289;
  ...
  ExpectedRuntimeQskSecondProbeBits =
    '3eb3ed67,3f007b40,3f1780a0,3f0e26cc,3eddd0a4,3e8492a0,'
    + '3f1c4001,3f0e267c,3e84916c,3eddcffc,3e849225,3d5ab707';
...
function ObserveRuntimeQskRun(
  const Seed: Integer;
  const EnableQskAfterFirstBlock: Boolean): TMonitorRunObservation;
var
  BlockIndex: Integer;
  BlockData: TSingleArray;
  ExpectedQsk: Boolean;
  ...
  try
    ...
    begin
      ...
      if (Length(BlockData) <> 1)
        ...
        raise Exception.Create('CE runtime QSK startup framing changed');
    end;
    ...
    if (Tst.Me.State <> stSending)
      ...
      raise Exception.Create('CE runtime QSK local CQ did not start');

    Result.MessageText := Tst.Me.MsgText;
    Result.EnvelopeSamples := Length(Tst.Me.Envelope);
    for BlockIndex := 0 to 1 do
    begin
      ExpectedQsk := EnableQskAfterFirstBlock and (BlockIndex = 1);
      if ExpectedQsk then
        Ini.Qsk := True;
      BlockData := Tst.GetAudio;
      if (Length(BlockData) <> ExpectedBlockSize)
        ...
        or Ini.Flutter
        or Ini.Lids
        or (Ini.Qsk <> ExpectedQsk)
        or (Round(MainForm.VolumeSlider1.Db)
            <> ExpectedMonitorLevelDb) then
        raise Exception.Create(
          'CE runtime QSK capture left its pinned path');
...
procedure ObserveRuntimeQskChange(
  const Values: TStrings;
  const Input: TJSONObject);
var
  BandwidthHz: Integer;
  ...
  ProbeIndexes: TIntegerArray;
  ProbeText: string;
  FixedQskOff: TMonitorRunObservation;
  RuntimeQskOn: TMonitorRunObservation;
  RuntimeQsk: Boolean;
  SampleRate: Integer;
  Seed: Integer;
  StationCall: string;
...
begin
  ...
  MonitorLevelDb := RequireInteger(
    ...
    -60,
    0);
  InitialQsk := RequireBoolean(Input, 'initialQsk');
  RuntimeQsk := RequireBoolean(Input, 'runtimeQsk');
  StationCall := RequireString(Input, 'stationCall');
  ...
  if (SampleRate <> ExpectedSampleRate)
    ...
    or (Length(ProbeIndexes) <> ExpectedProbeCount) then
    raise Exception.Create(
      'runtime QSK input does not match its pinned contract');
  for Index := 0 to ExpectedProbeCount - 1 do
    if ProbeIndexes[Index] <> ExpectedProbeSampleIndexes[Index] then
      raise Exception.Create(
        'runtime QSK probes do not match their pinned contract');

  FixedQskOff := ObserveRuntimeQskRun(Seed, False);
  RuntimeQskOn := ObserveRuntimeQskRun(Seed, True);
  if (FixedQskOff.MessageText <> ExpectedLocalMessage)
    or (RuntimeQskOn.MessageText <> ExpectedLocalMessage)
    or (FixedQskOff.EnvelopeSamples <> ExpectedQskEnvelopeSamples)
    or (FixedQskOff.EnvelopeSamples
        <> RuntimeQskOn.EnvelopeSamples)
    or (FixedQskOff.SendPosition <> ExpectedBlockSize * 2)
    or (RuntimeQskOn.SendPosition <> ExpectedBlockSize * 2)
    or (SingleBits(FixedQskOff.TerminalRandom) <> ExpectedTerminalBits)
    or (SingleBits(RuntimeQskOn.TerminalRandom)
        <> ExpectedTerminalBits) then
    raise Exception.Create('CE runtime QSK fixed observation changed');

  if not CompareMem(
      @FixedQskOff.Blocks[0][0],
      @RuntimeQskOn.Blocks[0][0],
      ExpectedBlockSize * SizeOf(Single)) then
    raise Exception.Create(
      'CE runtime QSK first block changed before mutation');
  Divergence := FirstDivergence(
    FixedQskOff.Blocks[1],
    RuntimeQskOn.Blocks[1]);
  if (Divergence <> ExpectedRuntimeQskFirstDivergence)
    or (FloatBlockSha256(FixedQskOff.Blocks[0])
        <> ExpectedFirstMonitorHash)
    or (FloatBlockSha256(FixedQskOff.Blocks[1])
        <> ExpectedFullSecondMonitorHash)
    or (FloatBlockSha256(RuntimeQskOn.Blocks[1])
        <> ExpectedRuntimeQskSecondHash)
    or (ProbeBits(FixedQskOff.Blocks[0], ProbeIndexes)
        <> ExpectedFullMonitorProbeBits)
    or (ProbeBits(FixedQskOff.Blocks[1], ProbeIndexes)
        <> ExpectedFullSecondMonitorProbeBits)
    or (ProbeBits(RuntimeQskOn.Blocks[1], ProbeIndexes)
        <> ExpectedRuntimeQskSecondProbeBits) then
    raise Exception.Create('CE runtime QSK audio observation changed');
  ProbeText := '';
  for Index := 0 to High(ProbeIndexes) do
  begin
    if Index > 0 then
  ...
  Values.Add(
    'local-message'
    + '|text=' + FixedQskOff.MessageText
    + '|rendered-local-samples=' + IntToStr(FixedQskOff.SendPosition)
    + '|probe-sample-indexes=' + ProbeText);
  AddBlockStatistics(
    'qsk-before-change-block[0]',
    FixedQskOff.Blocks[0],
    ProbeIndexes,
    Values);
  AddBlockStatistics(
  ...
  Values.Add(
    ...
    + FloatBlockSha256(FixedQskOff.Blocks[1])
    + '|qsk-runtime-on-float-sha256='
    + FloatBlockSha256(RuntimeQskOn.Blocks[1]));
  Values.Add(
    'terminal-random'
  ...
  if Values.Count <> ExpectedRuntimeQskValueCount then
    raise Exception.Create(
      'runtime QSK capture emitted an invalid row count');
end;
...
begin
  ...
  try
    ...
    ObserveRuntimeQskChange(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
