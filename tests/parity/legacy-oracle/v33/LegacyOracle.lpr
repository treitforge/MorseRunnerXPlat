...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v33';
  ExpectedSource =
    'tests/parity/legacy-oracle/v33/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v33/build-recipe.json';
  ExpectedScenario =
    'contest.arrldx-high-r1-power-remote-exchange-format-seed-12345';
  ExpectedStationCall = 'W7SST';
  ExpectedRemoteCall = 'K1ABC';
  ExpectedRst = 599;
  ExpectedArrlDxRemoteCall = 'JA1ABC';
  ExpectedCqwwZone = '10';
  ExpectedArrlDxPower = '100';
...
procedure ObserveFullCutNumericContest(
  const Values: TStrings;
...
begin
  ...
  try
    StationValue.MyCall := RemoteCall;
    ...
    StationValue.R1 := ExpectedR1Milli / 1000.0;
    StationValue.SentExchTypes := Tst.GetSentExchTypes(
      skDxStation,
      RemoteCall);
    RandSeed := ExpectedSeed;
    for Draw := 1 to ExpectedFormatterCheckpointDraw do
      Random;
...
procedure ObserveFullCutNumericRemoteExchangeFormat(
  const Values: TStrings;
  const Input: TJSONObject);
begin
  RequireExactObjectFields(
  ...
  if RequireString(Input, 'cqwwRemoteCall') <> ExpectedRemoteCall then
    raise Exception.Create('cqwwRemoteCall does not match the fixed vector');
  if RequireString(Input, 'arrlDxRemoteCall') <> ExpectedArrlDxRemoteCall then
    raise Exception.Create('arrlDxRemoteCall does not match the fixed vector');
  ...
  if RequireString(Input, 'cqwwZone') <> ExpectedCqwwZone then
    raise Exception.Create('cqwwZone does not match the fixed vector');
  if RequireString(Input, 'arrlDxPower') <> ExpectedArrlDxPower then
    raise Exception.Create('arrlDxPower does not match the fixed vector');
  if RequireInteger(Input, 'formatterCheckpointDraw', 0, MaxInt)
      <> ExpectedFormatterCheckpointDraw then
    raise Exception.Create(
      'formatterCheckpointDraw does not match the fixed vector');
  if RequireInteger(Input, 'r1Milli', 0, 1000) <> ExpectedR1Milli then
    raise Exception.Create('r1Milli does not match the fixed vector');
  ...
  Values.Add(
    ...
    + IntToStr(ExpectedFormatterCheckpointDraw)
    + '|r1-milli=' + IntToStr(ExpectedR1Milli)
    + '|run-mode=rmPileup');

  ObserveFullCutNumericContest(
    Values,
    Ord(scCQWW),
    'scCQWW',
    ExpectedRemoteCall,
  ...
  ObserveFullCutNumericContest(
    ...
    Ord(scArrlDx),
    'scArrlDx',
    ExpectedArrlDxRemoteCall,
    ExpectedArrlDxPower,
    1);

  if Values.Count <> 3 then
    raise Exception.Create(
      'full-cut numeric exchange scenario emitted invalid rows');
end;
...
begin
  ...
  try
    ...
    ObserveFullCutNumericRemoteExchangeFormat(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
