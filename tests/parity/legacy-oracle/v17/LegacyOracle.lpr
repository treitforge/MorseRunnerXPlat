...
{$ifndef V17_QRM_RETRY_LIFECYCLE}
  {$fatal V17_QRM_RETRY_LIFECYCLE must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v17';
  ExpectedSource =
    'tests/parity/legacy-oracle/v17/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v17/build-recipe.json';
  ExpectedScenario =
    'audio.qrm-first-station-retry-lifecycle-seed-1843';
  ...
  ExpectedPositiveQrmValueCount = 10;
  ...
  ExpectedLifecycleBlockCount = 367;
  ExpectedInitialCompletionBlock = 103;
  ExpectedSilentStartBlock = 104;
  ExpectedRetryTransitionBlock = 175;
  ExpectedRetryAudioStartBlock = 176;
  ExpectedRemovalBlock = 366;
  ExpectedLifecycleTerminalOrdinal = 376185;
  ...
  ExpectedLifecycleBoundary0 =
    'boundary|relative-block=0|absolute-block=6|state=stSending'
    + '|message=QRL?   QRL?|message-set=[msgQrl2]'
    + '|envelope-samples=53248|send-position=512'
    + '|timeout=2147483647'
    + '|block-sha256='
    + '72f7618e7e055db7fefd472c47f0488046087905d5810b1e1aa97b88187f643d';
  ExpectedLifecycleBoundary103 =
    'boundary|relative-block=103|absolute-block=109|state=stListening'
    + '|message=|message-set=[msgQrl2]|envelope-samples=0'
    + '|send-position=53248|timeout=72'
    + '|block-sha256='
    + '0b9e6d194be6e0f8da09abb0ee523ed534c6e8568b7dcb8c0ac773aa1aee30bd';
  ExpectedLifecycleBoundary175 =
    'boundary|relative-block=175|absolute-block=181|state=stSending'
    + '|message=CQ CQ TEST LU5MT LU5MT TEST'
    + '|message-set=[msgLongCQ]|envelope-samples=97792'
    + '|send-position=0|timeout=2147483647'
    + '|block-sha256='
    + 'a28259c95e95e35e351996ecfe0d980523b2eec5bb7ab7d58842d824d23c0ee3';
  ExpectedLifecycleRemoved =
    'removed|relative-block=366|absolute-block=372'
    + '|block-sha256='
    + '88b2b6ba40361e27b7eb757bc196c37c55c4e43fbf7ca295e7d8eca17432657e';
  ExpectedLifecycleTerminal =
    'terminal|value=0.036196634|single-bits=3d1442ec'
    + '|pick-station-calls=1|get-call-calls=1';
  ExpectedLifecycleSelectedAudio =
    'selected-audio|blocks=0,103,104,175,176,366'
    + '|hashes='
    + '72f7618e7e055db7fefd472c47f0488046087905d5810b1e1aa97b88187f643d,'
    + '0b9e6d194be6e0f8da09abb0ee523ed534c6e8568b7dcb8c0ac773aa1aee30bd,'
    + '9d514209cab95d3bd5c42b9cc471058201d0e16e993d3cdd05d8d49c89256ecb,'
    + 'a28259c95e95e35e351996ecfe0d980523b2eec5bb7ab7d58842d824d23c0ee3,'
    + 'a648393b7cf11d440f23ef388b79581e603ed79b1b643c27ec46feafec727ae8,'
    + '88b2b6ba40361e27b7eb757bc196c37c55c4e43fbf7ca295e7d8eca17432657e'
    + '|aggregate-sha256='
    + '5be3b60e7f13d67b2ccbbab4862d5546b05d987a0e167249443b0cd71735d1ac';
  ExpectedLifecycleAggregateAudio =
    'aggregate-audio|initial-sha256='
    + 'e76e61d591897fadde2339664240bc1cf09abf80ae223c668ff1c0cde64c4a39'
    + '|silent-sha256='
    + '3b126408de3fb40e27a16bc45a87e774e02a5b047a29aaabbd484a86ffa92da5'
    + '|retry-sha256='
    + '2676dc5c44ee976fd31877336b3a72a0ead802eecf5b79a83ed0f97e29015de2'
    + '|lifecycle-sha256='
    + '559854b62d8bd18fb98aebb11ebe1ffe665eaaeba58a4d37c4af6bb5e1e39496';
  ExpectedLifecycleReplay =
    'replay|trigger-count=367|trigger-bits-sha256='
    + '61ce8a81166e6100b46b011671588d33059eeb87fce2c10752f5a9d3a65ceaf1'
    + '|selected-blocks=0,103,104,175,176,366'
    + '|selected-trigger-bits='
    + '38e1bf40,3f157a95,3f52204c,3f6db3f8,3c997711,3f4d644d'
    + '|minimum-after-first-block=8'
    + '|minimum-after-first=0.005889148'
    + '|minimum-after-first-bits=3bc0f9c0'
    + '|timeout-input-bits=3f04a3db,3eb0d46d'
    + '|timeout-value=72.094093323'
    + '|timeout-value-bits=4290302d'
    + '|timeout-rounded=72|terminal-bits=3d1442ec';
...
function TStationAccess.ReadTimeout: Integer;
begin
  Result := TimeOut;
end;
...
begin
  ...
  if Values.Count <> ExpectedPositiveQrmValueCount then
    raise Exception.Create(
      'positive-QRM capture emitted an invalid row count');
end;

procedure ExploreReplay(const Values: TStrings);
var
  BlockIndex: Integer;
  Discarded: Extended;
  FirstTimeoutRandom: Single;
  HissIndex: Integer;
  MinimumAfterFirst: Single;
  MinimumAfterFirstBlock: Integer;
  SecondTimeoutRandom: Single;
  SelectedTriggerBits: string;
  TerminalRandom: Single;
  TimeoutValue: Single;
  Trigger: Single;
  TriggerBits: string;
begin
  ...
  SelectedTriggerBits := '';
  MinimumAfterFirst := 1;
  MinimumAfterFirstBlock := -1;
  FirstTimeoutRandom := 0;
  SecondTimeoutRandom := 0;
  for BlockIndex := 0 to 366 do
  begin
    for HissIndex := 0 to ExpectedBlockSize * 2 - 1 do
      Discarded := Random;
    Trigger := Random;
    if TriggerBits <> '' then
      TriggerBits := TriggerBits + ',';
    TriggerBits := TriggerBits + SingleBits(Trigger);
    if (BlockIndex = 0)
      or (BlockIndex = 103)
      or (BlockIndex = 104)
      or (BlockIndex = 175)
      or (BlockIndex = 176)
      or (BlockIndex = 366) then
    begin
      if SelectedTriggerBits <> '' then
        SelectedTriggerBits := SelectedTriggerBits + ',';
      SelectedTriggerBits :=
        SelectedTriggerBits + SingleBits(Trigger);
    end;
    if (BlockIndex > 0) and (Trigger < MinimumAfterFirst) then
    begin
      MinimumAfterFirst := Trigger;
      MinimumAfterFirstBlock := BlockIndex;
    end;
    if BlockIndex = 0 then
      for HissIndex := 0 to 7 do
        Discarded := Random;
    if BlockIndex = 103 then
    begin
      FirstTimeoutRandom := Random;
      SecondTimeoutRandom := Random;
    end;
  end;
  TerminalRandom := Random;

  RandSeed := ExpectedSeed;
  for HissIndex := 0 to 106607 do
    Discarded := Random;
  TimeoutValue := RndGaussLim(
    SecondsToBlocks(4),
    SecondsToBlocks(2));

  if Discarded < 0 then
    raise Exception.Create('lifecycle replay discarded value changed');
  Values.Add(
    'replay'
    + '|trigger-count=367'
    + '|trigger-bits-sha256='
    + Sha256Bytes(RawByteString(TriggerBits))
    + '|selected-blocks=0,103,104,175,176,366'
    + '|selected-trigger-bits=' + SelectedTriggerBits
    + '|minimum-after-first-block='
    + IntToStr(MinimumAfterFirstBlock)
    + '|minimum-after-first='
    + FloatValue(MinimumAfterFirst)
    + '|minimum-after-first-bits='
    + SingleBits(MinimumAfterFirst)
    + '|timeout-input-bits='
    + SingleBits(FirstTimeoutRandom)
    + ',' + SingleBits(SecondTimeoutRandom)
    + '|timeout-value=' + FloatValue(TimeoutValue)
    + '|timeout-value-bits=' + SingleBits(TimeoutValue)
    + '|timeout-rounded=' + IntToStr(Round(TimeoutValue))
    + '|terminal-bits=' + SingleBits(TerminalRandom));
end;

procedure ExploreLifecycle(
  const Values: TStrings;
  const Input: TJSONObject);
var
  AllAudio: TSingleArray;
  BlockData: TSingleArray;
  CompletionBlockHash: string;
  FinalBlockHash: string;
  FirstBlockHash: string;
  FirstRetryBlockHash: string;
  FirstSilentBlockHash: string;
  InitialAudio: TSingleArray;
  InputBandwidthHz: Integer;
  InputBlockSize: Integer;
  InputCallCatalogCount: Integer;
  InputInitialCompletionBlock: Integer;
  InputLifecycleBlockCount: Integer;
  InputMasterDataSha256: string;
  InputPitchHz: Integer;
  InputQrmTerminalOrdinal: Integer;
  InputQrmTriggerOrdinal: Integer;
  InputRemovalBlock: Integer;
  InputRetryAudioStartBlock: Integer;
  InputRetryTransitionBlock: Integer;
  InputSampleRate: Integer;
  InputSeed: Integer;
  InputSelectedCallIndex: Integer;
  InputSilentStartBlock: Integer;
  InputStartupRequestCount: Integer;
  InputStationCall: string;
  LastSilentBlockHash: string;
  NormalizedBlock: TSingleArray;
  PreviousState: TStationState;
  RelativeBlock: Integer;
  RetryAudio: TSingleArray;
  SampleIndex: Integer;
  SelectedAudio: TSingleArray;
  SelectedBlockIndexes: TIntegerArray;
  SilentAudio: TSingleArray;
  StartupIndex: Integer;
  Station: TStation;
  TerminalRandom: Single;

  procedure AppendBlock(
    var Destination: TSingleArray;
    const Source: TSingleArray);
  var
    DestinationOffset: Integer;
  begin
    DestinationOffset := Length(Destination);
    SetLength(
      Destination,
      DestinationOffset + Length(Source));
    if Length(Source) > 0 then
      Move(
        Source[0],
        Destination[DestinationOffset],
        Length(Source) * SizeOf(Single));
  end;

begin
  if Input = nil then
    raise Exception.Create('lifecycle exploration input is missing');
  RequireExactObjectFields(
    Input,
    [
      'bandwidthHz',
      ...
      'qrmTriggerRandomOrdinal',
      'removalBlock',
      'retryAudioStartBlock',
      'retryTransitionBlock',
      'sampleRate',
      'scenario',
      'seed',
  ...
  InputSampleRate := RequireInteger(
    Input, 'sampleRate', 1, MaxInt);
  InputBlockSize := RequireInteger(
    Input, 'blockSize', 1, MaxInt);
  InputSeed := RequireInteger(
    Input, 'seed', Low(Integer), MaxInt);
  InputBandwidthHz := RequireInteger(
    Input, 'bandwidthHz', 1, MaxInt);
  InputPitchHz := RequireInteger(
    Input, 'pitchHz', 1, MaxInt);
  InputStartupRequestCount := RequireInteger(
    Input, 'startupRequestCount', 0, MaxInt);
  InputLifecycleBlockCount := RequireInteger(
    Input, 'lifecycleBlockCount', 1, MaxInt);
  InputInitialCompletionBlock := RequireInteger(
    Input, 'initialCompletionBlock', 0, MaxInt);
  InputSilentStartBlock := RequireInteger(
    Input, 'silentStartBlock', 0, MaxInt);
  InputRetryTransitionBlock := RequireInteger(
    Input, 'retryTransitionBlock', 0, MaxInt);
  InputRetryAudioStartBlock := RequireInteger(
    Input, 'retryAudioStartBlock', 0, MaxInt);
  InputRemovalBlock := RequireInteger(
    Input, 'removalBlock', 0, MaxInt);
  InputQrmTriggerOrdinal := RequireInteger(
    Input, 'qrmTriggerRandomOrdinal', 0, MaxInt);
  InputQrmTerminalOrdinal := RequireInteger(
    Input, 'qrmTerminalRandomOrdinal', 0, MaxInt);
  InputCallCatalogCount := RequireInteger(
    Input, 'callCatalogCount', 1, MaxInt);
  InputSelectedCallIndex := RequireInteger(
    Input, 'selectedCallIndex', 0, MaxInt);
  InputMasterDataSha256 := RequireString(
    Input, 'masterDataSha256');
  InputStationCall := RequireString(Input, 'stationCall');
  SelectedBlockIndexes := RequireIntegerArray(
    Input,
    'selectedBlockIndexes',
    0,
    ExpectedLifecycleBlockCount - 1);

  if (InputSampleRate <> ExpectedSampleRate)
    or (InputBlockSize <> ExpectedBlockSize)
    or (InputSeed <> ExpectedSeed)
    or (InputBandwidthHz <> ExpectedBandwidthHz)
    or (InputPitchHz <> ExpectedPitchHz)
    or (InputStartupRequestCount
        <> ExpectedStartupRequestCount)
    or (InputLifecycleBlockCount
        <> ExpectedLifecycleBlockCount)
    or (InputInitialCompletionBlock
        <> ExpectedInitialCompletionBlock)
    or (InputSilentStartBlock <> ExpectedSilentStartBlock)
    or (InputRetryTransitionBlock
        <> ExpectedRetryTransitionBlock)
    or (InputRetryAudioStartBlock
        <> ExpectedRetryAudioStartBlock)
    or (InputRemovalBlock <> ExpectedRemovalBlock)
    or (InputQrmTriggerOrdinal
        <> ExpectedQrmTriggerOrdinal)
    or (InputQrmTerminalOrdinal
        <> ExpectedLifecycleTerminalOrdinal)
    or (InputCallCatalogCount <> ExpectedCatalogCount)
    or (InputSelectedCallIndex <> ExpectedCatalogIndex)
    or (InputMasterDataSha256
        <> ExpectedMasterDataSha256)
    or (InputStationCall <> 'W7SST')
    or (Length(SelectedBlockIndexes) <> 6)
    or (SelectedBlockIndexes[0] <> 0)
    or (SelectedBlockIndexes[1]
        <> ExpectedInitialCompletionBlock)
    or (SelectedBlockIndexes[2] <> ExpectedSilentStartBlock)
    or (SelectedBlockIndexes[3]
        <> ExpectedRetryTransitionBlock)
    or (SelectedBlockIndexes[4]
        <> ExpectedRetryAudioStartBlock)
    or (SelectedBlockIndexes[5] <> ExpectedRemovalBlock) then
    raise Exception.Create(
      'QRM lifecycle input does not match its pinned contract');

  CreateRuntime(ExpectedSeed, True);
  try
    RandSeed := ExpectedSeed;
    for StartupIndex := 0 to ExpectedStartupRequestCount - 1 do
    begin
      BlockData := Tst.GetAudio;
      if (Length(BlockData) <> 1) or (BlockData[0] <> 0) then
        raise Exception.Create('lifecycle startup framing changed');
    end;

    PreviousState := stListening;
    for RelativeBlock := 0 to 999 do
    begin
      BlockData := Tst.GetAudio;
      if Length(BlockData) <> ExpectedBlockSize then
        raise Exception.Create('lifecycle block length changed');
      SetLength(NormalizedBlock, ExpectedBlockSize);
      for SampleIndex := 0 to ExpectedBlockSize - 1 do
        NormalizedBlock[SampleIndex] :=
          NormalizeSample(BlockData[SampleIndex]);
      AppendBlock(AllAudio, NormalizedBlock);
      if RelativeBlock <= 103 then
        AppendBlock(InitialAudio, NormalizedBlock)
      else if RelativeBlock <= 175 then
        AppendBlock(SilentAudio, NormalizedBlock)
      else
        AppendBlock(RetryAudio, NormalizedBlock);
      if (RelativeBlock = 0)
        or (RelativeBlock = 103)
        or (RelativeBlock = 104)
        or (RelativeBlock = 175)
        or (RelativeBlock = 176)
        or (RelativeBlock = 366) then
        AppendBlock(SelectedAudio, NormalizedBlock);
      case RelativeBlock of
        0:
          FirstBlockHash := FloatBlockSha256(NormalizedBlock);
        103:
          CompletionBlockHash :=
            FloatBlockSha256(NormalizedBlock);
        104:
          FirstSilentBlockHash :=
            FloatBlockSha256(NormalizedBlock);
        175:
          LastSilentBlockHash :=
            FloatBlockSha256(NormalizedBlock);
      ...
      begin
        Values.Add(
          'removed'
          + '|relative-block=' + IntToStr(RelativeBlock)
          + '|absolute-block=' + IntToStr(Tst.BlockNumber)
          + '|block-sha256='
          + FloatBlockSha256(NormalizedBlock));
        Break;
      end;
      if Tst.Stations.Count <> 1 then
        raise Exception.Create(
          'lifecycle created an additional station');
      Station := Tst.Stations[0];
      if Station.ClassType <> TQrmStation then
        raise Exception.Create('lifecycle station class changed');

      if (RelativeBlock = 0)
        or ((PreviousState = stSending)
          and (Station.State <> stSending))
        or ((PreviousState <> stSending)
          and (Station.State = stSending)) then
        Values.Add(
          'boundary'
          + '|relative-block=' + IntToStr(RelativeBlock)
          + '|absolute-block=' + IntToStr(Tst.BlockNumber)
          + '|state=' + ToStr(Station.State)
          + '|message=' + Station.MsgText
          + '|message-set=' + ToStr(Station.Msg)
          + '|envelope-samples='
          + IntToStr(Length(Station.Envelope))
          + '|send-position='
          + IntToStr(
              TStationAccess(Station).ReadSendPosition)
          + '|timeout='
          + IntToStr(TStationAccess(Station).ReadTimeout)
          + '|block-sha256='
          + FloatBlockSha256(NormalizedBlock));
      PreviousState := Station.State;
    end;

    if Tst.Stations.Count <> 0 then
      raise Exception.Create(
        'lifecycle did not remove its first station');
    if (CurrentPickStationCalls <> 1)
      or (CurrentGetCallCalls <> 1) then
      raise Exception.Create(
        'lifecycle call selection count changed');
    TerminalRandom := Random;
    Values.Add(
      'terminal'
      ...
      + '|pick-station-calls='
      + IntToStr(CurrentPickStationCalls)
      + '|get-call-calls='
      + IntToStr(CurrentGetCallCalls));
    Values.Add(
      'selected-audio'
      + '|blocks=0,103,104,175,176,366'
      + '|hashes='
      + FirstBlockHash + ','
      + CompletionBlockHash + ','
      + FirstSilentBlockHash + ','
      ...
      + FinalBlockHash
      + '|aggregate-sha256='
      + FloatBlockSha256(SelectedAudio));
    Values.Add(
      'aggregate-audio'
      + '|initial-sha256='
      + FloatBlockSha256(InitialAudio)
      + '|silent-sha256='
      + FloatBlockSha256(SilentAudio)
      + '|retry-sha256='
      + FloatBlockSha256(RetryAudio)
      + '|lifecycle-sha256='
      + FloatBlockSha256(AllAudio));
  finally
    DestroyRuntime;
  end;
  ExploreReplay(Values);
  if (Values.Count <> ExpectedValueCount)
    or (Values[0] <> ExpectedLifecycleBoundary0)
    or (Values[1] <> ExpectedLifecycleBoundary103)
    or (Values[2] <> ExpectedLifecycleBoundary175)
    or (Values[3] <> ExpectedLifecycleRemoved)
    or (Values[4] <> ExpectedLifecycleTerminal)
    or (Values[5] <> ExpectedLifecycleSelectedAudio)
    or (Values[6] <> ExpectedLifecycleAggregateAudio)
    or (Values[7] <> ExpectedLifecycleReplay) then
    raise Exception.Create(
      'CE QRM lifecycle observation changed');
end;
