...
{$ifndef V55_AVALONIA_CUSTOM_WPM_STEP}
  {$fatal V55_AVALONIA_CUSTOM_WPM_STEP must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v55';
  ExpectedSource =
    'tests/parity/legacy-oracle/v55/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v55/build-recipe.json';
  ExpectedScenario =
    'ux.wpm-custom-page-up-command-step-7-wpm-seed-12345';
...
procedure ObserveCustomWpmPageUpStep(
  const Values: TStrings;
  const Input: TJSONObject);
var
  CustomStepWpm: Integer;
...
begin
  ...
  if (Seed <> ExpectedSeed)
    or (CustomStepWpm <> 7)
    or (InitialWpm <> 30)
    or (ExpectedAfterWpm <> 37)
    or (RunModeId <> 'rmSingle') then
    raise Exception.Create(
      'custom WPM PageUp input does not match its pinned contract');

  CreateRuntime(Seed, False);
  try
    Ini.RunMode := rmSingle;
    Ini.WpmStepRate := CustomStepWpm;
    MainForm.SetWpm(InitialWpm);
    if (Ini.Wpm <> InitialWpm)
      or (Ini.WpmStepRate <> CustomStepWpm) then
      raise Exception.Create(
        'CE custom WPM PageUp runtime did not start at its contract');
    Values.Add(
      'configuration'
      + '|run-mode=' + RunModeId
      + '|seed=' + IntToStr(Seed)
      + '|initial-wpm=' + IntToStr(InitialWpm)
      + '|custom-step-wpm=' + IntToStr(CustomStepWpm)
      + '|action=page-up'
    ...
    if (Key <> 0) or (Ini.Wpm <> ExpectedAfterWpm) then
      raise Exception.Create(
        'CE custom WPM PageUp handler reached an invalid value');
    Values.Add(
      'wpm-after-custom-page-up'
      + '|wpm=' + IntToStr(Ini.Wpm)
      + '|delta-wpm=' + IntToStr(Ini.Wpm - InitialWpm));
    if Values.Count <> 3 then
      raise Exception.Create(
        'custom WPM PageUp capture emitted an invalid row count');
  finally
    DestroyRuntime;
  end;
end;
...
begin
  ...
  try
    ...
    ObserveCustomWpmPageUpStep(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
