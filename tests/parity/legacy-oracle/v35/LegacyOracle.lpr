...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v35';
  ExpectedSource =
    'tests/parity/legacy-oracle/v35/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v35/build-recipe.json';
  ExpectedScenario =
  ...
  ExpectedRst = 599;
  ExpectedSerial = 123;
  ExpectedFormatterCheckpointDraw = 9;
  ExpectedR1Milli = 223;
  ExpectedSeed = 16;
...
procedure TOracleStation.ArmNumberError;
begin
  NrWithError := True;
end;
...
procedure ObserveLidSerialCorrectionContest(
  const Values: TStrings;
...
begin
  ...
  try
    for DrawIndex := 2 to 3 do
      Discarded := Random;
    Ini.Lids := True;
    Discarded := Random;
    StationValue.ArmNumberError;
    for DrawIndex := 5 to ExpectedFormatterCheckpointDraw do
      Discarded := Random;
    if Discarded < 0 then
      raise Exception.Create('invalid discarded random value');
    StationValue.MyCall := ExpectedRemoteCall;
    StationValue.HisCall := ExpectedStationCall;
    StationValue.RST := ExpectedRst;
    StationValue.NR := ExpectedSerial;
    StationValue.Exch1 := IntToStr(ExpectedRst);
    StationValue.Exch2 := IntToStr(ExpectedSerial);
    ...
    Values.Add(
      ...
      + '|exchange2=' + IntToStr(ExpectedSerial)
...
procedure ObserveLidSerialCorrectionRemoteExchangeFormat(
  const Values: TStrings;
  const Input: TJSONObject);
begin
  RequireExactObjectFields(
  ...
  if RequireString(Input, 'runModeId') <> 'rmWpx' then
    raise Exception.Create('runModeId does not match the fixed vector');
  if RequireInteger(Input, 'rst', 0, MaxInt) <> ExpectedRst then
    raise Exception.Create('rst does not match the fixed vector');
  if RequireInteger(Input, 'serial', 1, MaxInt) <> ExpectedSerial then
    raise Exception.Create('serial does not match the fixed vector');
  if not RequireBoolean(Input, 'lids') then
    raise Exception.Create('lids does not match the fixed vector');
  if RequireInteger(Input, 'formatterCheckpointDraw', 1, MaxInt)
  ...
  ObserveLidSerialCorrectionContest(
    Values,
    Ord(scWpx),
    'scWpx',
    0);
  ...
  if Values.Count <> 2 then
    raise Exception.Create(
      'LID serial correction scenario emitted invalid rows');
end;
...
begin
  ...
  try
    ...
    ObserveLidSerialCorrectionRemoteExchangeFormat(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
