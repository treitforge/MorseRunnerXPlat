...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v13';
  ExpectedSource =
    'tests/parity/legacy-oracle/v13/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v13/build-recipe.json';
  ExpectedScenario =
  ...
  ExpectedCompleteBlockCount = 1;
  ExpectedHissRandomDrawCount = 1024;
  ExpectedRandomCheckpointOrdinal = 1024;
  ExpectedRandomCheckpointSource = $2C80D4E4;
  ExpectedRandomCheckpointBits = '3e320354';
...
type
  ...
  TReceiverRunObservation = record
    Blocks: TSingleBlockArray;
    StationCount: Integer;
    RandomCheckpoint: Single;
  end;
...
function ObserveReceiverRun(
  const Seed: Integer;
  const BlockSize: Integer;
  const CompleteBlockCount: Integer): TReceiverRunObservation;
var
  BlockData: TSingleArray;
  BlockIndex: Integer;
  RandomCheckpointExact: Double;
  SampleIndex: Integer;
...
begin
  ...
  try
    ...
    SetLength(Result.Blocks, CompleteBlockCount);
    for BlockIndex := 0 to CompleteBlockCount - 1 do
    begin
      BlockData := Tst.GetAudio;
      if (Length(BlockData) <> BlockSize)
        ...
        or (AbstractStubCalls <> 0) then
        raise Exception.Create(
          'CE shared-random capture left the pure receiver path');
    ...
    if Result.StationCount <> 0 then
      raise Exception.Create(
        'CE shared-random capture created a station');

    RandomCheckpointExact := Random;
    Result.RandomCheckpoint := RandomCheckpointExact;
    if QWord(Trunc(RandomCheckpointExact * 4294967296.0))
        <> ExpectedRandomCheckpointSource then
      raise Exception.Create(
        'CE shared-random checkpoint source changed');
    if SingleBits(Result.RandomCheckpoint)
        <> ExpectedRandomCheckpointBits then
      raise Exception.Create(
        'CE shared-random checkpoint rounding changed');
  finally
    DestroyRuntime;
  end;
end;

procedure ObserveReceiverHissSharedRandomCheckpoint(
  const Values: TStrings;
  const Input: TJSONObject);
var
  BandwidthHz: Integer;
  BlockIndex: Integer;
  BlockSize: Integer;
  CompleteBlockCount: Integer;
  HissRandomDrawCount: Integer;
  PitchHz: Integer;
  ProbeIndex: Integer;
  ProbeSampleIndexes: TIntegerArray;
  ProbeText: string;
  RandomCheckpointOrdinal: Integer;
  ReceiverRun: TReceiverRunObservation;
  SampleRate: Integer;
  Seed: Integer;
...
begin
  ...
  StartupRequestCount := RequireInteger(
    ...
    1,
    MaxInt);
  CompleteBlockCount := RequireInteger(
    Input,
    'completeBlockCount',
    1,
    MaxInt);
  HissRandomDrawCount := RequireInteger(
    Input,
    'hissRandomDrawCount',
    1,
    MaxInt);
  RandomCheckpointOrdinal := RequireInteger(
    Input,
    'randomCheckpointOrdinal',
    1,
    MaxInt);
  ...
  if (SampleRate <> ExpectedSampleRate)
    ...
    or (CompleteBlockCount <> ExpectedCompleteBlockCount)
    or (HissRandomDrawCount <> ExpectedHissRandomDrawCount)
    or (RandomCheckpointOrdinal
        <> ExpectedRandomCheckpointOrdinal)
    or (Length(ProbeSampleIndexes) <> ExpectedProbeCount) then
    raise Exception.Create(
      'receiver hiss shared-random input does not match its contract');
  for ProbeIndex := 0 to ExpectedProbeCount - 1 do
    if ProbeSampleIndexes[ProbeIndex]
        <> ExpectedProbeSampleIndexes[ProbeIndex] then
      raise Exception.Create(
        'receiver hiss shared-random probes do not match their contract');
  ...
  Values.Add(
    ...
    + IntToStr(CompleteBlockCount)
    + '|hiss-random-draw-count='
    + IntToStr(HissRandomDrawCount)
    + '|random-checkpoint-ordinal='
    + IntToStr(RandomCheckpointOrdinal)
  ...
  ReceiverRun := ObserveReceiverRun(
    Seed,
    BlockSize,
    CompleteBlockCount);
  ...
  begin
    ...
    begin
      ...
      ProbeText := ProbeText
        + SingleBits(
            ReceiverRun.Blocks[BlockIndex][
              ProbeSampleIndexes[ProbeIndex]]);
    end;
    Values.Add(
      'receiver-block[' + IntToStr(BlockIndex) + ']'
      + '|sample-count=' + IntToStr(BlockSize)
      + '|probe-bits=' + ProbeText
      + '|float-sha256='
      + FloatBlockSha256(ReceiverRun.Blocks[BlockIndex]));
  end;
  ...
  Values.Add(
    ...
    + IntToStr(HissRandomDrawCount)
    + '|ordinal=' + IntToStr(RandomCheckpointOrdinal)
    + '|single-bits='
    + SingleBits(ReceiverRun.RandomCheckpoint));

  if Values.Count <> ExpectedValueCount then
    raise Exception.Create(
      'receiver hiss shared-random capture emitted an invalid row count');
end;
...
begin
  ...
  try
    ...
    ObserveReceiverHissSharedRandomCheckpoint(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
