...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v12';
  ExpectedSource =
    'tests/parity/legacy-oracle/v12/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v12/build-recipe.json';
  ExpectedScenario =
  ...
  try
    ...
    begin
      ...
      if (Length(BlockData) <> BlockSize)
        ...
        or (AbstractStubCalls <> 0) then
        raise Exception.Create(
          'CE qrm invariance capture left the pure receiver path');
    ...
    if Result.StationCount <> 0 then
      raise Exception.Create(
        'CE QRM no-trigger capture created a station');
  finally
    DestroyRuntime;
  end;
end;

procedure ObserveQrmNoTriggerInvariance(
  const Values: TStrings;
  const Input: TJSONObject);
var
  BandwidthHz: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      ProbeText := ProbeText
        + SingleBits(
            QrmRun.Blocks[BlockIndex][
              ProbeSampleIndexes[ProbeIndex]]);
    end;
    Values.Add(
      'qrm-block[' + IntToStr(BlockIndex) + ']'
      + '|sample-count=' + IntToStr(BlockSize)
      + '|probe-bits=' + ProbeText
      + '|float-sha256='
      + FloatBlockSha256(QrmRun.Blocks[BlockIndex]));
  end;

  Values.Add(
    'station-counts'
    + '|clean=' + IntToStr(Clean.StationCount)
    + '|qrm=' + IntToStr(QrmRun.StationCount));
  ...
  if Values.Count <> ExpectedValueCount then
    raise Exception.Create(
      'QRM no-trigger capture emitted an invalid row count');
end;
...
begin
  ...
  try
    ...
    ObserveQrmNoTriggerInvariance(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
