...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v20';
  ExpectedSource =
    'tests/parity/legacy-oracle/v20/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v20/build-recipe.json';
  ExpectedScenario =
...
procedure ObserveStartAndEmptyEnter(
  const Values: TStrings;
  const Input: TJSONObject);
var
  Key: Word;
...
begin
  ...
  CreateRuntime;
  if (Tst.Me.State <> stListening)
    or (Length(Tst.Me.Envelope) <> 0)
    or (Tst.Me.MsgText <> '')
    or (Tst.Me.Msg <> []) then
    raise Exception.Create('CE start boundary was not silent');
  Values.Add(
    'start-boundary'
  ...
  MainForm.FormKeyDown(MainForm, Key, []);
  if Key <> 0 then
    raise Exception.Create('CE did not consume empty VK_RETURN');
  if (Tst.Me.State <> stSending)
    or not (msgCQ in Tst.Me.Msg)
    or (Length(Tst.Me.Envelope) = 0) then
    raise Exception.Create('CE did not start the empty-Enter CQ');

  MessageText := Tst.Me.MsgText;
  Pieces := TStringList.Create;
  try
    DrainTransmission(Pieces);
    if (Pieces.Count <> 1)
      or (Pieces[0] <> MessageText) then
      raise Exception.Create('CE emitted an unexpected CQ piece queue');
  finally
    Pieces.Free;
  end;
  if MessageText <> 'CQ ' + ExpectedStationCall + ' TEST' then
    raise Exception.Create('CE emitted an unexpected CQ text');
  Values.Add(
    'empty-enter'
  ...
  if Values.Count <> 3 then
    raise Exception.Create(
      'start/empty-Enter scenario emitted an invalid row count');
end;
...
begin
  ...
  try
    ...
    ObserveStartAndEmptyEnter(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
