...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v11';
  ExpectedSource =
    'tests/parity/legacy-oracle/v11/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v11/build-recipe.json';
  ExpectedScenario =
...
procedure ConfigureIni(const FlutterEnabled: Boolean);
begin
  Ini.Call := 'W7SST';
  Ini.Wpm := 30;
...
function ObserveNoiseRun(
  ...
  const FlutterEnabled: Boolean): TNoiseRunObservation;
var
...
begin
  CreateRuntime(Seed, FlutterEnabled);
  try
    RandSeed := Seed;
    for StartupIndex := 0
    ...
    begin
      ...
      if (Length(BlockData) <> BlockSize)
        ...
        or (AbstractStubCalls <> 0) then
        raise Exception.Create(
          'CE flutter invariance capture left the pure receiver path');
...
procedure ObserveFlutterNoiseInvariance(
  const Values: TStrings;
  const Input: TJSONObject);
var
  BandwidthHz: Integer;
...
begin
  ...
  if (SampleRate <> ExpectedSampleRate)
    ...
    raise Exception.Create(
      'Flutter noise invariance input does not match its contract');
  for ProbeIndex := 0 to ExpectedProbeCount - 1 do
    if ProbeSampleIndexes[ProbeIndex]
        <> ExpectedProbeSampleIndexes[ProbeIndex] then
      raise Exception.Create(
        'Flutter noise invariance probes do not match their contract');
  ...
  begin
    ...
    begin
      ...
      ProbeText := ProbeText
        + SingleBits(
            Flutter.Blocks[BlockIndex][
              ProbeSampleIndexes[ProbeIndex]]);
    end;
    Values.Add(
      'flutter-block[' + IntToStr(BlockIndex) + ']'
      + '|sample-count=' + IntToStr(BlockSize)
      + '|probe-bits=' + ProbeText
      + '|float-sha256='
      + FloatBlockSha256(Flutter.Blocks[BlockIndex]));
  end;
  ...
  Values.Add(
    ...
    + '|flutter-float-sha256='
    + FloatBlockSha256(Flutter.Aggregate)
    + '|exact-equal='
    + LowerCase(BoolToStr(ExactEqual, True)));

  if Values.Count <> ExpectedValueCount then
    raise Exception.Create(
      'Flutter noise invariance emitted an invalid row count');
end;
...
begin
  ...
  try
    ...
    ObserveFlutterNoiseInvariance(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
