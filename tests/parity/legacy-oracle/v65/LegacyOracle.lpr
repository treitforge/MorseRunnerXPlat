...
const
  ExpectedScenario = 'contest.own-exchange-tokenization-boundaries';
  ExpectedAdapterId = 'LegacyOracleTarget';
  ExpectedVersionId = 'legacy-oracle-v65';
  ExpectedSource = 'tests/parity/legacy-oracle/v65/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v65/build-recipe.json';
...
begin
  ...
  try
    ...
    ErrorText := '';
    IsValid := ContestValue.ValidateMyExchange(
      Exchange,
      Tokens,
      ErrorText);
    Values.Add(
...
procedure ObserveExchangeVectors(
  const Values: TStrings;
...
begin
  ...
  try
    ...
    begin
      ...
      Exchange := RequireString(VectorValue, 'exchange');
...
begin
  ...
  try
    ObserveExchangeVectors(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
