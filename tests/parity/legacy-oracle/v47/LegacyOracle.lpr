...
{$ifndef V47_TUI_WPM_DEFAULT_PAGE_UP_STEP}
  {$fatal V47_TUI_WPM_DEFAULT_PAGE_UP_STEP must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v47';
  ExpectedSource =
    'tests/parity/legacy-oracle/v47/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v47/build-recipe.json';
  ExpectedScenario =
...
procedure ObserveDefaultWpmPageUpStep(
  const Values: TStrings;
  const Input: TJSONObject);
var
...
begin
  ...
  if (Seed <> ExpectedSeed)
    or (DefaultStepWpm <> 2)
    or (InitialWpm <> 30)
    or (ExpectedAfterWpm <> 32)
    or (RunModeId <> 'rmSingle') then
    raise Exception.Create(
      'default WPM PageUp input does not match its pinned contract');
  ...
  try
    ...
    if (Ini.Wpm <> InitialWpm)
      ...
      raise Exception.Create(
        'CE default WPM PageUp runtime did not start at its contract');
    Values.Add(
      'configuration'
      + '|run-mode=' + RunModeId
      + '|seed=' + IntToStr(Seed)
    ...
    if (Key <> 0) or (Ini.Wpm <> ExpectedAfterWpm) then
      raise Exception.Create(
        'CE default WPM PageUp handler reached an invalid value');
    Values.Add(
    ...
    if Values.Count <> 3 then
      raise Exception.Create(
        'default WPM PageUp capture emitted an invalid row count');
  finally
    DestroyRuntime;
  end;
end;
...
begin
  ...
  try
    ...
    ObserveDefaultWpmPageUpStep(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
