...
{$ifndef V45_TUI_RIT_DEFAULT_UP_STEP}
  {$fatal V45_TUI_RIT_DEFAULT_UP_STEP must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v45';
  ExpectedSource =
    'tests/parity/legacy-oracle/v45/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v45/build-recipe.json';
  ExpectedScenario =
...
procedure ObserveDefaultRitUpStep(
  const Values: TStrings;
  const Input: TJSONObject);
var
  DefaultStepHz: Integer;
  ExpectedAfterHz: Integer;
  RunModeId: string;
  Seed: Integer;
begin
  RequireExactObjectFields(
  ...
  Seed := RequireInteger(Input, 'seed', Low(Integer), MaxInt);
  DefaultStepHz := RequireInteger(
    Input,
    'defaultRitStepHz',
    1,
    500);
  ExpectedAfterHz := RequireInteger(
    Input,
    'expectedAfterHz',
    -500,
    500);
  ...
  if (Seed <> ExpectedSeed)
    or (DefaultStepHz <> ExpectedRitStepHz)
    or (ExpectedAfterHz <> ExpectedRitStepHz)
    or (RunModeId <> 'rmSingle') then
    raise Exception.Create(
      'default RIT Up input does not match its pinned contract');

  CreateRuntime(Seed, False);
  try
    Ini.RunMode := rmSingle;
    if (Ini.Rit <> 0)
      or (Ini.RitStepIncr <> DefaultStepHz) then
      raise Exception.Create(
        'CE default RIT Up runtime did not start at its defaults');
    Values.Add(
      'configuration'
      + '|run-mode=' + RunModeId
      + '|seed=' + IntToStr(Seed)
      + '|default-step-hz=' + IntToStr(DefaultStepHz)
      + '|action=up'
      + '|handler=TMainForm.Panel8MouseDown');
    Values.Add('rit-before|hz=' + IntToStr(Ini.Rit));
    MainForm.Panel8MouseDown(
      MainForm.Panel8,
      mbLeft,
      [],
      MainForm.Shape2.Left + MainForm.Shape2.Width + 1,
      0);
    if Ini.Rit <> ExpectedAfterHz then
      raise Exception.Create(
        'CE default RIT Up handler reached an invalid value');
    Values.Add(
      'rit-after-default-up'
      + '|hz=' + IntToStr(Ini.Rit)
      + '|delta-hz=' + IntToStr(Ini.Rit));
    if Values.Count <> 3 then
      raise Exception.Create(
        'default RIT Up capture emitted an invalid row count');
  finally
    DestroyRuntime;
  end;
end;
...
begin
  ...
  try
    ...
    ObserveDefaultRitUpStep(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
