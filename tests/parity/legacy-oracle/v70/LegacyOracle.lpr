...
{$ifndef V70_HST_START_CONSTRAINTS}
  {$fatal V70_HST_START_CONSTRAINTS must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v70';
  ExpectedSource =
    'tests/parity/legacy-oracle/v70/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v70/build-recipe.json';
  ExpectedScenario = 'session.hst-invalid-start-settings';
...
begin
  ...
  if (RequireString(Input, 'correctContestId') <> 'scHst')
    or (RequireString(Input, 'requiredSerialMode') <> 'snStartContest')
    or (RequireInteger(Input, 'seed', Low(Integer), MaxInt) <> 12345)
    or (RequireString(Input, 'wrongContestId') <> 'scWpx')
    or (RequireString(Input, 'wrongSerialMode') <> 'snMidContest') then
    raise Exception.Create(
  ...
  try
    ...
    MainForm.Run(rmHst);
    Values.Add(
      'hst-start-attempt|seed=12345|contest=scWpx|serial=snStartContest'
      + '|requested=rmHst|result=rmStop|accepted=false');
    if Ini.RunMode <> rmStop then
      raise Exception.Create(
        'CE accepted HST mode with a non-HST contest');
  finally
    FreeAndNil(MainForm);
  end;
  ...
  try
    Ini.RunMode := rmStop;
    Ini.SimContest := scHst;
    Ini.SerialNR := snMidContest;
    MainForm.Run(rmHst);
    Values.Add(
      'hst-start-attempt|seed=12345|contest=scHst|serial=snMidContest'
      + '|requested=rmHst|result=rmStop|accepted=false');
    if Ini.RunMode <> rmStop then
      raise Exception.Create(
        'CE accepted HST mode with a non-start serial range');
  finally
    FreeAndNil(MainForm);
  end;
...
begin
  ...
  try
    ...
    ObserveHstStartConstraints(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
