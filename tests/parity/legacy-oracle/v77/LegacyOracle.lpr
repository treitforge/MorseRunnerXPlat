...
{$ifndef V77_MALFORMED_OBSOLETE_SETTINGS}
  {$fatal V77_MALFORMED_OBSOLETE_SETTINGS must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v77';
  ExpectedSource =
    'tests/parity/legacy-oracle/v77/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v77/build-recipe.json';
  ExpectedScenario = 'settings.legacy-malformed-obsolete';
...
procedure ObserveMalformedAndObsoleteSettings(
  const Values: TStrings;
  const Input: TJSONObject);
var
  BandwidthHz: Integer;
  CwopsNumberExists: Boolean;
  IniFile: TIniFile;
  IniPath: string;
  PitchHz: Integer;
  Seed: Integer;
begin
  ...
  if Seed <> 12345 then
    raise Exception.Create(
      'malformed settings input does not match its pinned contract');

  IniPath := ChangeFileExt(ParamStr(0), '.ini');
  if FileExists(IniPath) and not DeleteFile(IniPath) then
    raise Exception.Create('could not clear the malformed settings INI path');
  IniFile := TIniFile.Create(IniPath);
  try
    IniFile.WriteString(SEC_STN, 'cwopsnum', '9999');
    IniFile.WriteString(SEC_STN, 'Pitch', 'not-a-value');
    IniFile.WriteString(SEC_STN, 'BandWidth', 'not-a-value');
    IniFile.WriteString(SEC_STN, 'Wpm', 'not-a-value');
    IniFile.WriteString(SEC_STN, 'Qsk', 'not-a-value');
    IniFile.WriteString(SEC_STN, 'SerialNR', 'not-a-value');
    IniFile.WriteString(SEC_STN, 'SelfMonVolume', 'not-a-value');
    IniFile.WriteString(SEC_STN, 'SaveWav', 'not-a-value');
    IniFile.WriteString(SEC_TST, 'SimContest', 'not-a-value');
    IniFile.WriteString(SEC_TST, 'DefaultRunMode', 'not-a-value');
    IniFile.WriteString(SEC_TST, 'Duration', 'not-a-value');
    IniFile.WriteString(SEC_TST, 'CompetitionDuration', 'not-a-value');
    IniFile.WriteString(SEC_SYS, 'BufSize', 'not-a-value');
    IniFile.WriteString(SEC_SYS, 'ShowCallsignInfo', 'not-a-value');
    IniFile.WriteString(SEC_SET, 'FarnsworthCharacterRate', 'not-a-value');
    IniFile.WriteString(SEC_SET, 'WpmStepRate', 'not-a-value');
    IniFile.WriteString(SEC_SET, 'RitStepIncr', 'not-a-value');
    IniFile.WriteString(SEC_SET, 'SingleCallStartDelay', 'not-a-value');
    IniFile.WriteString(SEC_BND, 'Activity', 'not-a-value');
    IniFile.WriteString(SEC_BND, 'Qsb', 'not-a-value');
    IniFile.WriteString(SEC_BND, 'Qrm', 'not-a-value');
    IniFile.WriteString(SEC_BND, 'Qrn', 'not-a-value');
    IniFile.WriteString(SEC_BND, 'Flutter', 'not-a-value');
    IniFile.WriteString(SEC_BND, 'Lids', 'not-a-value');
  finally
    IniFile.Free;
  end;
  ...
  try
    ...
    IniFile := TIniFile.Create(IniPath);
    try
      CwopsNumberExists := IniFile.ValueExists(SEC_STN, 'cwopsnum');
    finally
      IniFile.Free;
    end;
    Values.Add(
      'malformed-settings'
      + '|cwopsnum-exists=' + BoolToStr(CwopsNumberExists, True)
      + '|pitch-hz=' + IntToStr(PitchHz)
      + '|bandwidth-hz=' + IntToStr(BandwidthHz)
      + '|wpm=' + IntToStr(Ini.Wpm)
      + '|qsk=' + BoolToStr(Ini.Qsk, True)
      + '|serial=' + IntToStr(Ord(Ini.SerialNR))
      + '|monitor-db=' + IntToStr(Ini.MonLevel)
      + '|save-wav=' + BoolToStr(Ini.SaveWav, True)
      + '|contest=' + IntToStr(Ord(Ini.SimContest))
      + '|run-mode=' + IntToStr(Ord(Ini.DefaultRunMode))
      + '|duration=' + IntToStr(Ini.Duration)
      + '|competition-duration=' + IntToStr(Ini.CompDuration)
      + '|buffer-samples=' + IntToStr(Ini.BufSize)
      + '|show-call-info=' + BoolToStr(
          MainForm.mnuShowCallsignInfo.Checked, True)
      + '|farnsworth=' + IntToStr(Ini.FarnsworthCharRate)
      + '|wpm-step=' + IntToStr(Ini.WpmStepRate)
      + '|rit-step=' + IntToStr(Ini.RitStepIncr)
      + '|single-delay=' + IntToStr(Ini.SingleCallStartDelay)
      + '|activity=' + IntToStr(Ini.Activity)
      + '|qsb=' + BoolToStr(Ini.Qsb, True)
      + '|qrm=' + BoolToStr(Ini.Qrm, True)
      + '|qrn=' + BoolToStr(Ini.Qrn, True)
      + '|flutter=' + BoolToStr(Ini.Flutter, True)
      + '|lids=' + BoolToStr(Ini.Lids, True));
  finally
    FreeAndNil(MainForm);
    if FileExists(IniPath) and not DeleteFile(IniPath) then
      raise Exception.Create(
        'could not remove the malformed settings INI path');
  end;

  if Values.Count <> 1 then
    raise Exception.Create(
      'malformed settings capture emitted an invalid row count');
end;
...
begin
  ...
  try
    ...
    ObserveMalformedAndObsoleteSettings(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
