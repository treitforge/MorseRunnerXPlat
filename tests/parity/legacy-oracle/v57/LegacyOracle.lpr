...
{$ifndef V57_AVALONIA_CUSTOM_WPM_PAGE_DOWN}
  {$fatal V57_AVALONIA_CUSTOM_WPM_PAGE_DOWN must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v57';
  ExpectedSource =
    'tests/parity/legacy-oracle/v57/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v57/build-recipe.json';
  ExpectedScenario =
...
procedure ObserveCustomWpmPageDownStep(
  const Values: TStrings;
  const Input: TJSONObject);
var
...
begin
  ...
  if (Seed <> ExpectedSeed)
    or (CustomStepWpm <> 7)
    or (InitialWpm <> 30)
    or (ExpectedAfterWpm <> 23)
    or (RunModeId <> 'rmSingle') then
    raise Exception.Create(
      'custom WPM PageDown input does not match its pinned contract');
  ...
  try
    ...
    if (Ini.Wpm <> InitialWpm)
      ...
      raise Exception.Create(
        'CE custom WPM PageDown runtime did not start at its contract');
    Values.Add(
      'configuration'
      + '|run-mode=' + RunModeId
      + '|seed=' + IntToStr(Seed)
    ...
    if (Key <> 0) or (Ini.Wpm <> ExpectedAfterWpm) then
      raise Exception.Create(
        'CE custom WPM PageDown handler reached an invalid value');
    Values.Add(
    ...
    if Values.Count <> 3 then
      raise Exception.Create(
        'custom WPM PageDown capture emitted an invalid row count');
  finally
    DestroyRuntime;
  end;
end;
...
begin
  ...
  try
    ...
    ObserveCustomWpmPageDownStep(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
