...
{$ifndef V76_NRDIGITS_MIGRATION}
  {$fatal V76_NRDIGITS_MIGRATION must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v76';
  ExpectedSource =
    'tests/parity/legacy-oracle/v76/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v76/build-recipe.json';
  ExpectedScenario = 'settings.legacy-nrdigits-migration';
...
procedure ObserveLegacyNRDigitsMigration(
  const Values: TStrings;
  const Input: TJSONObject);
var
  InputValue: Integer;
...
begin
  ...
  if Seed <> 12345 then
    raise Exception.Create(
      'legacy NRDigits input does not match its pinned contract');

  IniPath := ChangeFileExt(ParamStr(0), '.ini');
  for InputValue := 0 to 5 do
  begin
    if FileExists(IniPath) and not DeleteFile(IniPath) then
      raise Exception.Create('could not clear the NRDigits INI path');
    IniFile := TIniFile.Create(IniPath);
    try
      IniFile.WriteInteger(SEC_STN, 'NRDigits', InputValue);
    finally
      IniFile.Free;
    end;
    ...
    try
      ...
      IniFile := TIniFile.Create(IniPath);
      try
        LegacyKeyExists := IniFile.ValueExists(SEC_STN, 'NRDigits');
        PersistedSerial := IniFile.ReadInteger(SEC_STN, 'SerialNR', -1);
      finally
        IniFile.Free;
      end;
      if LegacyKeyExists or (PersistedSerial <> Ord(Ini.SerialNR)) then
        raise Exception.Create('CE NRDigits migration behavior changed');
      Values.Add(
        'legacy-nrdigits'
        + '|input=' + IntToStr(InputValue)
        + '|legacy-key-exists=' + BoolToStr(LegacyKeyExists, True)
        + '|serial=' + IntToStr(PersistedSerial));
    finally
      FreeAndNil(MainForm);
    end;
  end;
  ...
  if FileExists(IniPath) and not DeleteFile(IniPath) then
    raise Exception.Create('could not remove the NRDigits INI path');

  if Values.Count <> 6 then
    raise Exception.Create(
      'legacy NRDigits capture emitted an invalid row count');
end;
...
begin
  ...
  try
    ...
    ObserveLegacyNRDigitsMigration(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
