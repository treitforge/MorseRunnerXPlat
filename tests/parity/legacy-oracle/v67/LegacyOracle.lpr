...
const
  ExpectedScenario = 'contest.jarl-call-history-truth-column-mapping';
  ExpectedAdapterId = 'LegacyOracleTarget';
  ExpectedVersionId = 'legacy-oracle-v67';
  ExpectedSource = 'tests/parity/legacy-oracle/v67/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v67/build-recipe.json';
...
begin
  ...
  try
    RequireExactObjectFields(Result, ['scenario', 'seed', 'vectors']);
...
begin
  ...
  if (Data.AsInt64 < Low(Integer)) or (Data.AsInt64 > High(Integer)) then
    raise Exception.Create(Name + ' is outside the integer range');
...
begin
  ...
  try
    ...
    if not ContestValue.OnContestPrepareToStart('W7SST', '') then
      raise Exception.Create('Contest call history did not load');
    RandSeed := Seed;
    StationIndex := ContestValue.PickStation;
    CallValue := ContestValue.GetCall(StationIndex);
    if ContestId = scAllJa then
    begin
      Exchange1 := TALLJA(ContestValue).getExch1(StationIndex);
      Exchange2 := TALLJA(ContestValue).getExch2(StationIndex);
    end
    else
    begin
      Exchange1 := TACAG(ContestValue).getExch1(StationIndex);
      Exchange2 := TACAG(ContestValue).getExch2(StationIndex);
    end;
    Values.Add(
...
procedure ObserveJarlTruthVectors(
  const Values: TStrings;
...
begin
  ...
  Seed := RequireInteger(Input, 'seed');
  ...
  try
    ...
    begin
      ...
      RequireExactObjectFields(VectorValue, ['contestId']);
      ...
      if not (TSimContest(ContestOrdinal) in [scAllJa, scAcag]) then
...
begin
  ...
  try
    try
      ObserveJarlTruthVectors(Values, Input);
      Emit(
        Scenario,
        AdapterId,
        VersionId,
