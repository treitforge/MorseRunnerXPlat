...
{$ifndef V49_TUI_WPM_SETUP_UPPER_RANGE}
  {$fatal V49_TUI_WPM_SETUP_UPPER_RANGE must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v49';
  ExpectedSource =
    'tests/parity/legacy-oracle/v49/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v49/build-recipe.json';
  ExpectedScenario =
...
procedure ObserveTuiWpmSetupUpperRange(
  const Values: TStrings;
  const Input: TJSONObject);
var
  ExpectedAfterWpm: Integer;
  ExpectedMaximumWpm: Integer;
  ExpectedMinimumWpm: Integer;
  InitialWpm: Integer;
  Seed: Integer;
  SettingsIncrementWpm: Integer;
begin
  RequireExactObjectFields(
    Input,
    [
  ...
  ExpectedAfterWpm := RequireInteger(
    ...
    10,
    120);
  ExpectedMaximumWpm := RequireInteger(
    Input,
    'expectedMaximumWpm',
    10,
    120);
  ExpectedMinimumWpm := RequireInteger(
    Input,
    'expectedMinimumWpm',
    10,
    120);
  InitialWpm := RequireInteger(Input, 'initialWpm', 10, 120);
  SettingsIncrementWpm := RequireInteger(
    Input,
    'settingsIncrementWpm',
    1,
    20);
  if (Seed <> ExpectedSeed)
    or (InitialWpm <> 100)
    or (SettingsIncrementWpm <> 1)
    or (ExpectedAfterWpm <> 101)
    or (ExpectedMinimumWpm <> 10)
    or (ExpectedMaximumWpm <> 120) then
    raise Exception.Create(
      'TUI WPM setup upper-range input does not match its pinned contract');

  CreateRuntime(Seed, False);
  try
    MainForm.SetWpm(InitialWpm);
    if (Ini.Wpm <> InitialWpm)
      or (MainForm.SpinEdit1.Value <> InitialWpm)
      or (MainForm.SpinEdit1.MinValue <> ExpectedMinimumWpm)
      or (MainForm.SpinEdit1.MaxValue <> ExpectedMaximumWpm) then
      raise Exception.Create(
        'CE WPM setup range did not start at its contract');
    Values.Add(
      'configuration'
      + '|seed=' + IntToStr(Seed)
      + '|initial-wpm=' + IntToStr(InitialWpm)
      + '|settings-increment-wpm=' + IntToStr(SettingsIncrementWpm)
      + '|control-min-wpm=' + IntToStr(MainForm.SpinEdit1.MinValue)
      + '|control-max-wpm=' + IntToStr(MainForm.SpinEdit1.MaxValue)
      + '|handler=TMainForm.SetWpm');
    Values.Add('wpm-before|wpm=' + IntToStr(Ini.Wpm));
    MainForm.SetWpm(Ini.Wpm + SettingsIncrementWpm);
    if (Ini.Wpm <> ExpectedAfterWpm)
      or (MainForm.SpinEdit1.Value <> ExpectedAfterWpm) then
      raise Exception.Create(
        'CE WPM setup increment did not enter the upper range');
    Values.Add(
    ...
    if Values.Count <> 3 then
      raise Exception.Create(
        'TUI WPM setup upper-range capture emitted an invalid row count');
  finally
    DestroyRuntime;
  end;
end;
...
begin
  ...
  try
    ...
    ObserveTuiWpmSetupUpperRange(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
