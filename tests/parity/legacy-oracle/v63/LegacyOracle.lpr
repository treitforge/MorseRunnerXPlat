...
const
  ExpectedScenario = 'catalog.contest-definition-metadata-ce-order';
  ExpectedAdapterId = 'LegacyOracleTarget';
  ExpectedVersionId = 'legacy-oracle-v63';
  ExpectedSource = 'tests/parity/legacy-oracle/v63/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v63/build-recipe.json';
...
procedure ObserveContestDefinitions(const Values: TStrings);
var
  ContestId: TSimContest;
  Definition: TContestDefinition;
begin
  for ContestId := Low(TSimContest) to High(TSimContest) do
  begin
    Definition := ContestDefinitions[ContestId];
    if Definition.T <> ContestId then
      raise Exception.Create('Contest definition order mismatch');
    Values.Add(
      'contest|ordinal=' + IntToStr(Ord(ContestId))
      + '|id=' + GetEnumName(TypeInfo(TSimContest), Ord(ContestId))
      + '|key=' + Definition.Key
      + '|name=' + Definition.Name
      + '|exchange1=' + GetEnumName(
        TypeInfo(TExchange1Type), Ord(Definition.ExchType1))
      + '|exchange2=' + GetEnumName(
        TypeInfo(TExchange2Type), Ord(Definition.ExchType2))
      + '|caption1=' + Definition.ExchCaptions[0]
      + '|caption2=' + Definition.ExchCaptions[1]
      + '|editable=' + LowerCase(
        BoolToStr(Definition.ExchFieldEditable, True))
      + '|default=' + Definition.ExchDefault
...
begin
  ...
  try
    ObserveContestDefinitions(Values);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
