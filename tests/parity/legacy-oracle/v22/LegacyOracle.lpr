...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v22';
  ExpectedSource =
    'tests/parity/legacy-oracle/v22/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v22/build-recipe.json';
  ExpectedScenario =
...
procedure ObserveCwtRemoteExchangeFormat(
  const Values: TStrings;
  const Input: TJSONObject);
var
...
begin
  ...
  if RequireString(Input, 'contestId') <> 'scCwt' then
    raise Exception.Create('contestId does not match the fixed vector');
  ...
  if Values.Count <> 2 then
    raise Exception.Create('CWT exchange scenario emitted invalid rows');
end;
...
begin
  ...
  try
    ...
    ObserveCwtRemoteExchangeFormat(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
