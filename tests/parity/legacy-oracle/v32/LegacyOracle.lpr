...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v32';
  ExpectedSource =
    'tests/parity/legacy-oracle/v32/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v32/build-recipe.json';
  ExpectedScenario =
    'contest.jarl-random-cut-remote-exchange-format-seed-12345';
  ExpectedStationCall = 'W7SST';
  ExpectedRemoteCall = 'JA1ABC';
  ExpectedRst = 599;
  ExpectedAllJaExchange2 = '109H';
  ExpectedAcagExchange2 = '1009H';
  ExpectedFormatterCheckpointDraw = 3;
  ExpectedSeed = 12345;
...
procedure ObserveJarlRandomCutContest(
  const Values: TStrings;
...
procedure ObserveJarlRandomCutRemoteExchangeFormat(
  const Values: TStrings;
  const Input: TJSONObject);
begin
  RequireExactObjectFields(
  ...
  ObserveJarlRandomCutContest(
    Values,
    Ord(scAllJa),
    'scAllJa',
    ExpectedAllJaExchange2,
    0);
  ObserveJarlRandomCutContest(
    Values,
    Ord(scAcag),
    'scAcag',
    ExpectedAcagExchange2,
    1);

  if Values.Count <> 3 then
    raise Exception.Create(
      'JARL random-cut exchange scenario emitted invalid rows');
end;
...
begin
  ...
  try
    ...
    ObserveJarlRandomCutRemoteExchangeFormat(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
