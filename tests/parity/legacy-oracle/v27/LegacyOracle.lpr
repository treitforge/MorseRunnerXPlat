...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v27';
  ExpectedSource =
    'tests/parity/legacy-oracle/v27/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v27/build-recipe.json';
  ExpectedScenario =
    'contest.wpx-custom-range-remote-exchange-format-seed-12345';
  ExpectedStationCall = 'W7SST';
  ExpectedRemoteCall = 'K1ABC';
  ExpectedRst = 599;
  ExpectedExchange1 = '599';
  ExpectedExchange2 = '7';
  ExpectedCustomRange = '01-99';
  ExpectedSeed = 12345;
...
procedure ObserveWpxCustomRangeRemoteExchangeFormat(
  const Values: TStrings;
  const Input: TJSONObject);
var
  ErrorText: string;
...
begin
  ...
  if RequireString(Input, 'contestId') <> 'scWpx' then
    raise Exception.Create('contestId does not match the fixed vector');
  if RequireString(Input, 'runModeId') <> 'rmWpx' then
    raise Exception.Create('runModeId does not match the fixed vector');
  if RequireString(Input, 'customRange') <> ExpectedCustomRange then
    raise Exception.Create('customRange does not match the fixed vector');
  if RequireInteger(Input, 'rst', 0, MaxInt) <> ExpectedRst then
    raise Exception.Create('rst does not match the fixed vector');
  if RequireString(Input, 'exchange1') <> ExpectedExchange1 then
    raise Exception.Create('exchange1 does not match the fixed vector');
  if RequireString(Input, 'exchange2') <> ExpectedExchange2 then
    raise Exception.Create('exchange2 does not match the fixed vector');
  ...
  Values.Add(
    ...
    + '|custom-range=' + ExpectedCustomRange);

  CreateRuntime(Ord(scWpx));
  Ini.SerialNR := snCustomRange;
  ErrorText := '';
  if not Ini.SerialNRSettings[snCustomRange].ParseSerialNR(
      ExpectedCustomRange,
      ErrorText) then
    raise Exception.Create(
      'CE rejected the fixed custom serial range: ' + ErrorText);
  ...
  try
    ...
    StationValue.NR := StrToInt(ExpectedExchange2);
    StationValue.R1 := 0.0;
    StationValue.Exch1 := ExpectedExchange1;
    StationValue.Exch2 := ExpectedExchange2;
    ...
    Values.Add(
      ...
      + '|exchange1=' + ExpectedExchange1
      + '|exchange2=' + ExpectedExchange2
  ...
  if Values.Count <> 2 then
    raise Exception.Create(
      'WPX custom-range exchange scenario emitted invalid rows');
end;
...
begin
  ...
  try
    ...
    ObserveWpxCustomRangeRemoteExchangeFormat(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
