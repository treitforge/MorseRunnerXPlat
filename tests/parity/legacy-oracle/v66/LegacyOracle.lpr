...
const
  ExpectedScenario = 'contest.dynamic-exchange-type-locality-matrix';
  ExpectedAdapterId = 'LegacyOracleTarget';
  ExpectedVersionId = 'legacy-oracle-v66';
  ExpectedSource = 'tests/parity/legacy-oracle/v66/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v66/build-recipe.json';
...
begin
  if Value.Count <> Length(ExpectedNames) then
    raise Exception.Create('JSON object has unsupported fields');
  ...
  begin
    ...
    if not Found then
      raise Exception.Create(
        'JSON object has unsupported field: ' + Value.Names[Index]);
  end;
end;
...
begin
  ...
  try
    RequireExactObjectFields(Result, ['scenario', 'vectors']);
    if RequireString(Result, 'scenario') <> Scenario then
...
procedure ObserveOneVector(
  const Values: TStrings;
  const VectorOrdinal: Integer;
  const ContestId: TSimContest;
  const ContestName: string;
  const HomeCall: string;
  const StationKind: TStationKind;
  const StationKindName: string;
  const RequestedMsgType: TRequestedMsgType;
  const RequestedMsgTypeName: string;
  const StationCall: string;
  const RemoteCall: string);
var
...
begin
  ...
  Tst := ContestValue;
  try
    if not ContestValue.OnSetMyCall(HomeCall, ErrorText) then
      raise Exception.Create(
    ...
    ExchangeTypes := ContestValue.GetExchangeTypes(
      ...
      RemoteCall);
    Values.Add(
      'vector|ordinal=' + IntToStr(VectorOrdinal)
      + '|contest=' + ContestName
      + '|homeCall=' + HomeCall
      + '|stationKind=' + StationKindName
      + '|requestedMsgType=' + RequestedMsgTypeName
      + '|stationCall=' + StationCall
      + '|remoteCall=' + RemoteCall
      + '|exch1=' + GetEnumName(
        TypeInfo(TExchange1Type), Ord(ExchangeTypes.Exch1))
      + '|exch2=' + GetEnumName(
        TypeInfo(TExchange2Type), Ord(ExchangeTypes.Exch2)));
  finally
    Tst := nil;
    ContestValue.Free;
  end;
...
procedure ObserveExchangeTypeVectors(
  const Values: TStrings;
  const Input: TJSONObject);
var
  ContestName: string;
  ContestId: TSimContest;
  ContestOrdinal: Integer;
  Data: TJSONData;
  HomeCall: string;
  Index: Integer;
  RemoteCall: string;
  RequestedMsgType: TRequestedMsgType;
  RequestedMsgTypeName: string;
  RequestedMsgTypeOrdinal: Integer;
  StationCall: string;
  StationKind: TStationKind;
  StationKindName: string;
  StationKindOrdinal: Integer;
  VectorData: TJSONData;
  VectorValue: TJSONObject;
  Vectors: TJSONArray;
begin
  Data := Input.Find('vectors');
  if (Data = nil) or (Data.JSONType <> jtArray) then
    raise Exception.Create('vectors is not an array');
  Vectors := TJSONArray(Data);
  if Vectors.Count = 0 then
    raise Exception.Create('vectors is empty');
  gDXCCList := TDXCC.Create;
  try
    for Index := 0 to Vectors.Count - 1 do
    begin
      VectorData := Vectors.Items[Index];
      if not (VectorData is TJSONObject) then
        raise Exception.Create('vector is not an object');
      VectorValue := TJSONObject(VectorData);
      ...
      ContestName := RequireString(VectorValue, 'contestId');
      HomeCall := RequireString(VectorValue, 'homeCall');
      StationKindName := RequireString(VectorValue, 'stationKind');
      RequestedMsgTypeName := RequireString(
        VectorValue,
        'requestedMsgType');
      StationCall := RequireString(VectorValue, 'stationCall');
      RemoteCall := RequireString(VectorValue, 'remoteCall');
      ContestOrdinal := GetEnumValue(TypeInfo(TSimContest), ContestName);
      if (ContestOrdinal < Ord(Low(TSimContest)))
        or (ContestOrdinal > Ord(High(TSimContest)))
        or (GetEnumName(TypeInfo(TSimContest), ContestOrdinal)
          <> ContestName) then
        raise Exception.Create('vector has invalid contestId');
      if not (TSimContest(ContestOrdinal) in [scArrlDx, scNaQp]) then
        raise Exception.Create('vector has unsupported contestId');
      ContestId := TSimContest(ContestOrdinal);
      StationKindOrdinal := GetEnumValue(
        TypeInfo(TStationKind),
        StationKindName);
      if (StationKindOrdinal < Ord(Low(TStationKind)))
        or (StationKindOrdinal > Ord(High(TStationKind)))
        or (GetEnumName(TypeInfo(TStationKind), StationKindOrdinal)
          <> StationKindName) then
        raise Exception.Create('vector has invalid stationKind');
      StationKind := TStationKind(StationKindOrdinal);
      RequestedMsgTypeOrdinal := GetEnumValue(
        TypeInfo(TRequestedMsgType),
        RequestedMsgTypeName);
      if (RequestedMsgTypeOrdinal < Ord(Low(TRequestedMsgType)))
        or (RequestedMsgTypeOrdinal > Ord(High(TRequestedMsgType)))
        or (GetEnumName(
          TypeInfo(TRequestedMsgType),
          RequestedMsgTypeOrdinal) <> RequestedMsgTypeName) then
        raise Exception.Create('vector has invalid requestedMsgType');
      RequestedMsgType := TRequestedMsgType(RequestedMsgTypeOrdinal);
      ObserveOneVector(
        Values,
        Index,
        ContestId,
...
begin
  ...
  try
    ObserveExchangeTypeVectors(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
