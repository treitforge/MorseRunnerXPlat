...
{$ifndef V37_QSK_RECEIVER_DUCKING}
  {$fatal V37_QSK_RECEIVER_DUCKING must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v37';
  ExpectedSource =
    'tests/parity/legacy-oracle/v37/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v37/build-recipe.json';
  ExpectedScenario =
  ...
  ExpectedQskFirstDivergence = 310;
  ExpectedQskOffHash =
  ...
  ExpectedQskOffProbeBits =
    '00000000,00000000,00000000,00000000,00000000,00000000,'
    + '00000000,af1ef2fe,bc1821a0,be84929e,bd5ab8c9,3e22613d';
  ExpectedQskOnProbeBits =
    '00000000,00000000,00000000,00000000,00000000,00000000,'
    + '00000000,b64a5608,bc15edf7,be84922a,bd5ab758,3e2260e7';
...
function ObserveQskRun(
  const Seed: Integer;
  const QskEnabled: Boolean): TQskRunObservation;
var
  BlockData: TSingleArray;
  SampleIndex: Integer;
  StartupIndex: Integer;
  ...
  try
    ...
    begin
      ...
      if (Length(BlockData) <> 1)
        ...
        raise Exception.Create('CE QSK startup framing changed');
    end;
    ...
    if (Tst.Me.State <> stSending)
      ...
      raise Exception.Create('CE QSK local CQ did not start');
    ...
    if (Length(BlockData) <> ExpectedBlockSize)
      ...
      or Ini.Flutter
      or Ini.Lids
      or (Ini.Qsk <> QskEnabled)
      or (Round(MainForm.VolumeSlider1.Db)
          <> ExpectedMonitorLevelDb) then
      raise Exception.Create('CE QSK capture left its pinned path');
...
procedure ObserveQskReceiverDucking(
  const Values: TStrings;
  const Input: TJSONObject);
var
  BandwidthHz: Integer;
  ...
  ProbeIndexes: TIntegerArray;
  ProbeText: string;
  QskOff: TQskRunObservation;
  QskOn: TQskRunObservation;
  SampleRate: Integer;
  Seed: Integer;
  StationCall: string;
  StartupRequestCount: Integer;
...
begin
  ...
  if (SampleRate <> ExpectedSampleRate)
    ...
    or (Length(ProbeIndexes) <> ExpectedProbeCount) then
    raise Exception.Create('QSK input does not match its pinned contract');
  for Index := 0 to ExpectedProbeCount - 1 do
    if ProbeIndexes[Index] <> ExpectedProbeSampleIndexes[Index] then
      raise Exception.Create('QSK probes do not match their pinned contract');

  QskOff := ObserveQskRun(Seed, False);
  QskOn := ObserveQskRun(Seed, True);
  if (QskOff.MessageText <> ExpectedLocalMessage)
    or (QskOn.MessageText <> ExpectedLocalMessage)
    or (QskOff.EnvelopeSamples <> ExpectedQskEnvelopeSamples)
    or (QskOff.EnvelopeSamples <> QskOn.EnvelopeSamples)
    or (QskOff.SendPosition <> ExpectedBlockSize)
    or (QskOn.SendPosition <> ExpectedBlockSize)
    or (SingleBits(QskOff.TerminalRandom) <> ExpectedTerminalBits)
    or (SingleBits(QskOn.TerminalRandom) <> ExpectedTerminalBits) then
    raise Exception.Create('CE QSK fixed observation changed');

  Divergence := FirstDivergence(QskOff.Block, QskOn.Block);
  if (Divergence <> ExpectedQskFirstDivergence)
    or (FloatBlockSha256(QskOff.Block) <> ExpectedQskOffHash)
    or (FloatBlockSha256(QskOn.Block) <> ExpectedQskOnHash)
    or (ProbeBits(QskOff.Block, ProbeIndexes)
        <> ExpectedQskOffProbeBits)
    or (ProbeBits(QskOn.Block, ProbeIndexes)
        <> ExpectedQskOnProbeBits) then
    raise Exception.Create('CE QSK audio observation changed');
  ProbeText := '';
  for Index := 0 to High(ProbeIndexes) do
  begin
    if Index > 0 then
  ...
  Values.Add(
    'local-message'
    + '|text=' + QskOff.MessageText
    + '|rendered-local-samples=' + IntToStr(QskOff.SendPosition)
  ...
  Values.Add(
    ...
    + '|exact-equal=false'
    + '|first-divergence=' + IntToStr(Divergence)
    + '|qsk-off-float-sha256=' + FloatBlockSha256(QskOff.Block)
    + '|qsk-on-float-sha256=' + FloatBlockSha256(QskOn.Block));
  Values.Add(
    'terminal-random'
    + '|ordinal=' + IntToStr(TerminalRandomOrdinal)
    + '|qsk-off-single-bits=' + SingleBits(QskOff.TerminalRandom)
    + '|qsk-on-single-bits=' + SingleBits(QskOn.TerminalRandom));

  if Values.Count <> ExpectedQskValueCount then
    raise Exception.Create('QSK capture emitted an invalid row count');
end;
...
begin
  ...
  try
    ...
    ObserveQskReceiverDucking(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
