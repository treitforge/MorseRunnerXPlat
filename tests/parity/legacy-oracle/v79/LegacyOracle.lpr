...
{$ifndef V79_NIL_WITHOUT_LIVE_STATION_TRUTH}
  {$fatal V79_NIL_WITHOUT_LIVE_STATION_TRUTH must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v79';
  ExpectedSource =
    'tests/parity/legacy-oracle/v79/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v79/build-recipe.json';
  ExpectedScenario =
...
procedure ObserveNilWithoutLiveStationTruth(
  const Values: TStrings;
  const Input: TJSONObject);
var
  Qso: PQso;
...
begin
  RequireExactObjectFields(
    Input,
    ['scenario', 'seed', 'contestId', 'call', 'rst', 'number']);
  Seed := RequireInteger(Input, 'seed', Low(Integer), MaxInt);
  if (Seed <> 12345)
    or (RequireString(Input, 'contestId') <> 'scWpx')
    or (RequireString(Input, 'call') <> 'K1ABC')
    or (RequireInteger(Input, 'rst', 0, 999) <> 599)
    or (RequireInteger(Input, 'number', 0, 9999) <> 1) then
    raise Exception.Create(
      'NIL logging input does not match its pinned contract');

  if Assigned(Tst) or Assigned(MainForm) or (QsoList <> nil) then
    raise Exception.Create('CE NIL logging globals were not initially clear');
  ...
  Tst := TContest.Create;
  try
    MainForm.RecvExchTypes := Tst.GetRecvExchTypes(
      skMyStation,
      Ini.Call,
      'K1ABC');
    SetLength(QsoList, 1);
    ...
    CheckErr;
    if (Qso.ExchError <> leNIL) or (Qso.Err <> 'NIL') then
      raise Exception.Create('CE did not classify missing truth as NIL');
    ...
    Values.Add(
      ...
      + '|rst=' + IntToStr(Qso.Rst)
      + '|true-rst=' + IntToStr(Qso.TrueRst)
      + '|number=' + IntToStr(Qso.Nr)
      + '|true-number=' + IntToStr(Qso.TrueNr)
      + '|exchange-error=leNIL'
      + '|error-text=' + Qso.Err);
  finally
    QsoList := nil;
    FreeAndNil(Tst);
  ...
  if Values.Count <> 2 then
    raise Exception.Create('NIL logging capture emitted an invalid row count');
end;
...
begin
  ...
  try
    ...
    ObserveNilWithoutLiveStationTruth(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
