...
{$ifndef V61_AVALONIA_WPM_STEP_UPPER_CLAMP}
  {$fatal V61_AVALONIA_WPM_STEP_UPPER_CLAMP must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v61';
  ExpectedSource =
    'tests/parity/legacy-oracle/v61/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v61/build-recipe.json';
  ExpectedScenario =
    'ux.wpm-step-upper-clamp-page-up-from-21-seed-12345';
...
begin
  ...
  if FindWSRegistered(TMenu) = nil then
    RegisterWSComponent(TMenu, TWSMenu);
  if FindWSRegistered(TPopupMenu) = nil then
...
procedure ObserveWpmStepUpperClamp(
  const Values: TStrings;
  const Input: TJSONObject);
var
  EffectiveStepWpm: Integer;
  ExpectedAfterWpm: Integer;
  InitialWpm: Integer;
  IniFile: TIniFile;
  IniPath: string;
  Key: Word;
  PersistedStepWpm: Integer;
  RunModeId: string;
  Seed: Integer;
begin
  RequireExactObjectFields(
  ...
  Seed := RequireInteger(Input, 'seed', Low(Integer), MaxInt);
  PersistedStepWpm := RequireInteger(
    Input,
    'persistedStepWpm',
    Low(Integer),
    MaxInt);
  EffectiveStepWpm := RequireInteger(
    Input,
    'effectiveStepWpm',
    1,
    20);
  ExpectedAfterWpm := RequireInteger(
    Input,
    'expectedAfterWpm',
    10,
    120);
  ...
  if (Seed <> ExpectedSeed)
    or (PersistedStepWpm <> 21)
    or (EffectiveStepWpm <> 20)
    or (InitialWpm <> 30)
    or (ExpectedAfterWpm <> 50)
    or (RunModeId <> 'rmSingle') then
    raise Exception.Create(
      'WPM step upper-clamp input does not match its pinned contract');

  IniPath := ChangeFileExt(ParamStr(0), '.ini');
  if FileExists(IniPath) then
    raise Exception.Create('CE oracle INI path was not initially clear');
  try
    IniFile := TIniFile.Create(IniPath);
    try
      IniFile.WriteInteger('Settings', 'WpmStepRate', PersistedStepWpm);
    finally
      IniFile.Free;
    end;
    ...
    try
      ...
      on E: Exception do
        raise Exception.Create(
          'CE INI-load form construction failed: ' + E.Message);
    end;
    try
      try
        Ini.FromIni(@RaiseIniError);
      except
        on E: Exception do
          raise Exception.Create('CE FromIni failed: ' + E.Message);
      end;
      if Ini.WpmStepRate <> EffectiveStepWpm then
        raise Exception.Create(
          'CE persisted WPM step did not clamp to its upper bound');
    finally
      FreeAndNil(MainForm);
    end;
    ...
    if (Ini.Wpm <> InitialWpm)
      or (Ini.WpmStepRate <> EffectiveStepWpm) then
      raise Exception.Create(
        'CE WPM step upper-clamp runtime did not start at its contract');
    Values.Add(
      'configuration'
      + '|run-mode=' + RunModeId
      + '|seed=' + IntToStr(Seed)
      + '|initial-wpm=' + IntToStr(InitialWpm)
      + '|persisted-step-wpm=' + IntToStr(PersistedStepWpm)
      + '|effective-step-wpm=' + IntToStr(EffectiveStepWpm)
      + '|action=page-up'
      + '|handler=TMainForm.FormKeyDown');
    Values.Add('wpm-before|wpm=' + IntToStr(Ini.Wpm));
    Key := VK_PRIOR;
    MainForm.FormKeyDown(MainForm, Key, []);
    if (Key <> 0) or (Ini.Wpm <> ExpectedAfterWpm) then
      raise Exception.Create(
        'CE upper-clamped WPM step handler reached an invalid value');
    Values.Add(
    ...
    if Values.Count <> 3 then
      raise Exception.Create(
        'WPM step upper-clamp capture emitted an invalid row count');
  finally
    DestroyRuntime;
    if FileExists(IniPath) and not DeleteFile(IniPath) then
      raise Exception.Create('CE oracle INI cleanup failed');
  end;
end;
...
begin
  ...
  try
    ...
    ObserveWpmStepUpperClamp(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
