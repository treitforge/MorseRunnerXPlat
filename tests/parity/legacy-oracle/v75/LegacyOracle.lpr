...
{$ifndef V75_PRESERVE_LEGACY_VALUES}
  {$fatal V75_PRESERVE_LEGACY_VALUES must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v75';
  ExpectedSource =
    'tests/parity/legacy-oracle/v75/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v75/build-recipe.json';
  ExpectedScenario = 'settings.preserve-legacy-values';
...
procedure ObservePreserveLegacyValues(
  const Values: TStrings;
  const Input: TJSONObject);
var
  IniFile: TIniFile;
  IniPath: string;
  PreservedCall: string;
  PreservedUnknown: string;
  PreservedUnconsumed: Boolean;
  Seed: Integer;
begin
  RequireExactObjectFields(Input, ['scenario', 'seed']);
  Seed := RequireInteger(Input, 'seed', Low(Integer), MaxInt);
  if Seed <> 12345 then
    raise Exception.Create(
      'preserve legacy values input does not match its pinned contract');

  IniPath := ChangeFileExt(ParamStr(0), '.ini');
  if FileExists(IniPath) and not DeleteFile(IniPath) then
    raise Exception.Create('could not clear the preservation INI path');
  IniFile := TIniFile.Create(IniPath);
  try
    IniFile.WriteString('Future', 'Mystery', 'keep-me');
    IniFile.WriteBool(SEC_STN, 'CallsFromKeyer', True);
    IniFile.WriteString(SEC_STN, 'Call', 'K7ABC');
  finally
    IniFile.Free;
  end;
  ...
  try
    FromIni(RaiseIniError);
    if not Ini.CallsFromKeyer then
      raise Exception.Create('CE did not load the unconsumed setting');
    Ini.Call := 'K7XYZ';
    ToIni;
    IniFile := TIniFile.Create(IniPath);
    try
      PreservedUnknown := IniFile.ReadString('Future', 'Mystery', '');
      PreservedUnconsumed :=
        IniFile.ReadBool(SEC_STN, 'CallsFromKeyer', False);
      PreservedCall := IniFile.ReadString(SEC_STN, 'Call', '');
    finally
      IniFile.Free;
    end;
    if (PreservedUnknown <> 'keep-me')
      or not PreservedUnconsumed
      or (PreservedCall <> 'K7XYZ') then
      raise Exception.Create('CE INI preservation behavior changed');
    Values.Add(
      'preservation'
      + '|unknown=' + PreservedUnknown
      + '|unconsumed=' + BoolToStr(PreservedUnconsumed, True)
      + '|call=' + PreservedCall);
  finally
    FreeAndNil(MainForm);
    if FileExists(IniPath) and not DeleteFile(IniPath) then
      raise Exception.Create('could not remove the preservation INI path');
  end;

  if Values.Count <> 1 then
    raise Exception.Create(
      'legacy value preservation capture emitted an invalid row count');
end;
...
begin
  ...
  try
    ...
    ObservePreserveLegacyValues(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
