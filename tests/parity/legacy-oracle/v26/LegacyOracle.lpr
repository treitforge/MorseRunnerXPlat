...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v26';
  ExpectedSource =
    'tests/parity/legacy-oracle/v26/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v26/build-recipe.json';
  ExpectedScenario =
    'contest.fieldday-remote-exchange-format-seed-12345';
  ExpectedStationCall = 'W7SST';
  ExpectedRemoteCall = 'K1ABC';
  ExpectedRst = 599;
  ExpectedExchange1 = '3A';
  ExpectedExchange2 = 'OR';
  ExpectedSeed = 12345;
...
procedure ObserveFieldDayRemoteExchangeFormat(
  const Values: TStrings;
  const Input: TJSONObject);
var
...
begin
  ...
  if RequireString(Input, 'contestId') <> 'scFieldDay' then
    raise Exception.Create('contestId does not match the fixed vector');
  ...
  if Values.Count <> 2 then
    raise Exception.Create('Field Day exchange scenario emitted invalid rows');
end;
...
begin
  ...
  try
    ...
    ObserveFieldDayRemoteExchangeFormat(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
