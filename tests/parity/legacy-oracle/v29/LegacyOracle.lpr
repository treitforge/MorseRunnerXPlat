...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v29';
  ExpectedSource =
    'tests/parity/legacy-oracle/v29/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v29/build-recipe.json';
  ExpectedScenario =
    'contest.sweepstakes-remote-exchange-format-seed-12345';
  ExpectedStationCall = 'W7SST';
  ExpectedRemoteCall = 'K1ABC';
  ExpectedRst = 599;
  ExpectedExchange1 = '123 A';
  ExpectedExchange2 = '72 OR';
  ExpectedSeed = 12345;
...
procedure ObserveSweepstakesRemoteExchangeFormat(
  const Values: TStrings;
  const Input: TJSONObject);
var
...
begin
  ...
  if RequireString(Input, 'contestId') <> 'scArrlSS' then
    raise Exception.Create('contestId does not match the fixed vector');
  ...
  if Values.Count <> 2 then
    raise Exception.Create('Sweepstakes exchange scenario emitted invalid rows');
end;
...
begin
  ...
  try
    ...
    ObserveSweepstakesRemoteExchangeFormat(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
