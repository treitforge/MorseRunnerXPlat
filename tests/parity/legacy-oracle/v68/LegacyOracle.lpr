...
const
  ExpectedScenario = 'audio.wav-pcm16-bit-exact';
  ExpectedAdapterId = 'LegacyOracleTarget';
  ExpectedVersionId = 'legacy-oracle-v68';
  ExpectedSource = 'tests/parity/legacy-oracle/v68/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v68/build-recipe.json';
...
function BytesToHex(const Bytes: TByteBuffer): string;
var
  Index: Integer;
begin
  Result := '';
  for Index := 0 to High(Bytes) do
    Result := Result + LowerCase(IntToHex(Bytes[Index], 2));
end;

procedure ObserveWavPcm16(
  const Values: TStrings;
...
var
  ...
  SamplesJson: TJSONArray;
  TempPath: string;
  Wav: TAlWavFile;
begin
  Data := Input.Find('pcmSamples');
  if (Data = nil) or (Data.JSONType <> jtArray) then
    raise Exception.Create('pcmSamples is not an array');
  SamplesJson := TJSONArray(Data);
  if SamplesJson.Count = 0 then
    raise Exception.Create('pcmSamples is empty');
  SampleRate := RequireInteger(Input, 'sampleRate');
  if SampleRate <= 0 then
    raise Exception.Create('sampleRate is not positive');
  SetLength(Samples, SamplesJson.Count);
  for Index := 0 to SamplesJson.Count - 1 do
  begin
    if not (SamplesJson.Items[Index] is TJSONIntegerNumber) then
      raise Exception.Create('PCM sample is not an integer');
    if (SamplesJson.Items[Index].AsInt64 < -32767)
      or (SamplesJson.Items[Index].AsInt64 > 32767) then
      raise Exception.Create('PCM sample is outside the CE range');
    Samples[Index] := SamplesJson.Items[Index].AsInt64;
  end;

  TempPath := GetTempFileName(GetTempDir(False), 'mrp');
  Wav := TAlWavFile.Create(nil);
  try
    Wav.FileName := TempPath;
    Wav.Stereo := False;
    Wav.SamplesPerSec := SampleRate;
    Wav.BytesPerSample := 2;
    Wav.OpenWrite;
    try
      Wav.WriteFrom(@Samples[0], nil, Length(Samples));
    finally
      Wav.Close;
    end;
    ...
    try
      if FileStream.Size > High(Integer) then
        raise Exception.Create('WAV output is too large');
      SetLength(Bytes, Integer(FileStream.Size));
    ...
    Values.Add(
      'wav|sampleRate=' + IntToStr(SampleRate)
      + '|sampleCount=' + IntToStr(Length(Samples))
