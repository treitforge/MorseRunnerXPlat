...
{$ifndef V19_QSB_RUNTIME_TOGGLE}
  {$fatal V19_QSB_RUNTIME_TOGGLE must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v19';
  ExpectedSource =
    'tests/parity/legacy-oracle/v19/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v19/build-recipe.json';
  ExpectedScenario =
    'audio.qsb-runtime-toggle-active-station-seed-12345';
  ExpectedSampleRate = 11025;
  ExpectedBlockSize = 512;
  ExpectedSeed = 12345;
  ExpectedComparedBlockCount = 4;
  ExpectedToggleAfterBlockCount = 2;
  ExpectedValueCount = 15;
  ExpectedProbeCount = 6;
  ExpectedStationCall = 'K1ABC';
  ExpectedMessageText =
    'K1ABC K1ABC K1ABC K1ABC K1ABC K1ABC K1ABC K1ABC';
...
function TQsbContest.LoadCallHistory(
  const AUserCallsign: string): Boolean;
begin
  Result := AUserCallsign = Ini.Call;
end;

function TQsbContest.PickStation: Integer;
begin
  Inc(PickStationCalls);
  Result := 1;
end;

procedure TQsbContest.DropStation(Id: Integer);
begin
  raise Exception.Create(
    'QSB runtime-toggle case must not drop a station');
end;

function TQsbContest.GetCall(Id: Integer): string;
begin
  if Id <> 1 then
    raise Exception.Create('QSB station ID changed');
  Inc(GetCallCalls);
  Result := ExpectedStationCall;
end;

procedure TQsbContest.GetExchange(
  Id: Integer;
  out AStation: TDxStation);
begin
  if (Id <> 1) or not Assigned(AStation) then
    raise Exception.Create('QSB exchange target changed');
  Inc(GetExchangeCalls);
  AStation.RST := 599;
  AStation.NR := 123;
  AStation.Exch1 := 'Randy';
  AStation.Exch2 := 'WA';
  AStation.OpName := 'Randy';
end;
...
begin
  ...
  MainForm.SetBw((Ini.Bandwidth - 100) div 50);

  if (Tst.ClassType <> TQsbContest)
    ...
    or (Ini.Bandwidth <> 500)
    or Ini.Qsb
    or (MainForm.ComboBox2.ItemIndex <> 8)
    or (Tst.Filt.Points <> 15)
    or (Tst.Filt2.Points <> Tst.Filt.Points)
    or (PickStationCalls <> 0)
    or (GetCallCalls <> 0)
    or (GetExchangeCalls <> 0) then
    raise Exception.Create('CE QSB station runtime is not active');
end;
...
function ObserveQsbRun(
  const Seed: Integer;
  const BlockSize: Integer;
  const ComparedBlockCount: Integer;
  const ToggleAfterBlockCount: Integer;
  const RuntimeToggle: Boolean): TQsbRunObservation;
var
...
begin
  CreateRuntime(Seed);
  try
    Station := Tst.Stations.AddCaller as TDxStation;
    if not Assigned(Station)
      or (Tst.Stations.Count <> 1)
      or (Station.MyCall <> ExpectedStationCall)
      or (PickStationCalls <> 1)
      or (GetCallCalls <> 1)
      or (GetExchangeCalls <> 1)
      or Ini.Qsb then
      raise Exception.Create(
        'CE QSB caller construction changed');

    Station.WpmS := 30;
    Station.WpmC := 30;
    Station.Amplitude := 1;
    Station.Pitch := 0;
    Station.SendText(ExpectedMessageText);
    if (Station.State <> stSending)
      or (Length(Station.Envelope)
          < ComparedBlockCount * BlockSize) then
      raise Exception.Create(
        'CE QSB station message is too short');
    ...
    begin
      Ini.Qsb :=
        RuntimeToggle
        and (BlockIndex >= ToggleAfterBlockCount);
      BlockData := Station.GetBlock;
      if (Length(BlockData) <> BlockSize)
        or (Tst.Stations.Count <> 1)
        or (Station.State <> stSending)
        or (Ini.Qsb
            <> (RuntimeToggle
                and (BlockIndex >= ToggleAfterBlockCount))) then
        raise Exception.Create(
          'CE QSB runtime-toggle capture changed');
...
procedure AddBlockRows(
  const Values: TStrings;
  const Prefix: string;
  const Run: TQsbRunObservation;
...
var
  ...
  ProbeText: string;
begin
  for BlockIndex := 0 to High(Run.Blocks) do
  begin
    ProbeText := '';
    ...
    begin
      ...
      ProbeText := ProbeText
        + SingleBits(
            Run.Blocks[BlockIndex][
              ProbeSampleIndexes[ProbeIndex]]);
    end;
    Values.Add(
      Prefix + '-block[' + IntToStr(BlockIndex) + ']'
      + '|sample-count='
      + IntToStr(Length(Run.Blocks[BlockIndex]))
      + '|probe-bits=' + ProbeText
      + '|float-sha256='
      + FloatBlockSha256(Run.Blocks[BlockIndex]));
  end;
end;

procedure ObserveQsbRuntimeToggle(
  const Values: TStrings;
  const Input: TJSONObject);
var
  BlockIndex: Integer;
  ...
  SampleRate: Integer;
  Seed: Integer;
  ToggleAfterBlockCount: Integer;
  Toggled: TQsbRunObservation;
begin
  ...
  ComparedBlockCount := RequireInteger(
    ...
    1,
    MaxInt);
  ToggleAfterBlockCount := RequireInteger(
    Input,
    'toggleAfterBlockCount',
    1,
  ...
  if (SampleRate <> ExpectedSampleRate)
    ...
    or (ToggleAfterBlockCount
        <> ExpectedToggleAfterBlockCount)
    or (MessageText <> ExpectedMessageText)
    or (Length(ProbeSampleIndexes) <> ExpectedProbeCount) then
    raise Exception.Create(
      'QSB runtime-toggle input does not match its contract');
  for ProbeIndex := 0 to ExpectedProbeCount - 1 do
    if ProbeSampleIndexes[ProbeIndex]
        <> ExpectedProbeSampleIndexes[ProbeIndex] then
      raise Exception.Create(
        'QSB runtime-toggle probes do not match their contract');
  ...
  begin
    ExactEqual := SingleArraysEqual(
      ...
      Toggled.Blocks[BlockIndex]);
    Values.Add(
      'runtime-transition[' + IntToStr(BlockIndex) + ']'
      + '|qsb-enabled='
      + LowerCase(
          BoolToStr(
            BlockIndex >= ToggleAfterBlockCount,
            True))
      + '|disabled-float-sha256='
      + FloatBlockSha256(Disabled.Blocks[BlockIndex])
      + '|runtime-toggle-float-sha256='
      + FloatBlockSha256(Toggled.Blocks[BlockIndex])
      + '|exact-equal='
  ...
  Values.Add(
    ...
    + '|sample-count='
    + IntToStr(Length(Disabled.Aggregate))
    + '|disabled-float-sha256='
    + FloatBlockSha256(Disabled.Aggregate)
    + '|runtime-toggle-float-sha256='
    + FloatBlockSha256(Toggled.Aggregate)
    + '|exact-equal='
    + LowerCase(BoolToStr(ExactEqual, True)));

  Values.Add(
    'terminal-random'
    + '|disabled-value=' + FloatValue(Disabled.TerminalRandom)
    + '|disabled-single-bits='
    + SingleBits(Disabled.TerminalRandom)
    + '|runtime-toggle-value='
    + FloatValue(Toggled.TerminalRandom)
    + '|runtime-toggle-single-bits='
    + SingleBits(Toggled.TerminalRandom));

  if Values.Count <> ExpectedValueCount then
    raise Exception.Create(
      'QSB runtime-toggle case emitted an invalid row count');
end;
...
begin
  ...
  try
    ...
    ObserveQsbRuntimeToggle(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
