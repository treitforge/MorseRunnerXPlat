...
{$ifndef V50_TUI_WPM_SETUP_LOWER_CLAMP}
  {$fatal V50_TUI_WPM_SETUP_LOWER_CLAMP must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v50';
  ExpectedSource =
    'tests/parity/legacy-oracle/v50/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v50/build-recipe.json';
  ExpectedScenario =
...
procedure ObserveTuiWpmSetupLowerClamp(
  const Values: TStrings;
  const Input: TJSONObject);
var
...
begin
  ...
  SettingsDecrementWpm := RequireInteger(
    Input,
    'settingsDecrementWpm',
    1,
    20);
  if (Seed <> ExpectedSeed)
    or (InitialWpm <> 10)
    or (SettingsDecrementWpm <> 1)
    or (ExpectedAfterWpm <> 10)
    or (ExpectedMinimumWpm <> 10)
    or (ExpectedMaximumWpm <> 120) then
    raise Exception.Create(
      'TUI WPM setup lower-clamp input does not match its pinned contract');
  ...
  try
    ...
    if (Ini.Wpm <> InitialWpm)
      ...
      raise Exception.Create(
        'CE WPM setup lower clamp did not start at its contract');
    Values.Add(
      'configuration'
      + '|seed=' + IntToStr(Seed)
      + '|initial-wpm=' + IntToStr(InitialWpm)
      + '|settings-decrement-wpm=' + IntToStr(SettingsDecrementWpm)
    ...
    MainForm.SetWpm(Ini.Wpm - SettingsDecrementWpm);
    if (Ini.Wpm <> ExpectedAfterWpm)
      or (MainForm.SpinEdit1.Value <> ExpectedAfterWpm) then
      raise Exception.Create(
        'CE WPM setup decrement escaped the lower clamp');
    Values.Add(
    ...
    if Values.Count <> 3 then
      raise Exception.Create(
        'TUI WPM setup lower-clamp capture emitted an invalid row count');
  finally
    DestroyRuntime;
  end;
end;
...
begin
  ...
  try
    ...
    ObserveTuiWpmSetupLowerClamp(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
