...
{$ifndef V54_TUI_HST_WPM_PAGE_DOWN_ROUNDING}
  {$fatal V54_TUI_HST_WPM_PAGE_DOWN_ROUNDING must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v54';
  ExpectedSource =
    'tests/parity/legacy-oracle/v54/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v54/build-recipe.json';
  ExpectedScenario =
...
procedure ObserveHstWpmPageDownRounding(
  const Values: TStrings;
  const Input: TJSONObject);
var
...
begin
  ...
  if (Seed <> ExpectedSeed)
    or (InitialWpm <> 33)
    or (ExpectedAfterWpm <> 30)
    or (RunModeId <> 'rmHst') then
    raise Exception.Create(
      'HST WPM PageDown input does not match its pinned contract');
  ...
  try
    ...
    if Ini.Wpm <> InitialWpm then
      raise Exception.Create(
        'CE HST WPM PageDown runtime did not start at its contract');
    Values.Add(
      'configuration'
      + '|run-mode=' + RunModeId
      + '|seed=' + IntToStr(Seed)
    ...
    if (Key <> 0) or (Ini.Wpm <> ExpectedAfterWpm) then
      raise Exception.Create(
        'CE HST WPM PageDown handler reached an invalid value');
    Values.Add(
    ...
    if Values.Count <> 3 then
      raise Exception.Create(
        'HST WPM PageDown capture emitted an invalid row count');
  finally
    DestroyRuntime;
  end;
end;
...
begin
  ...
  try
    ...
    ObserveHstWpmPageDownRounding(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
