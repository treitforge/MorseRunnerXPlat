...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v21';
  ExpectedSource =
    'tests/parity/legacy-oracle/v21/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v21/build-recipe.json';
  ExpectedScenario =
...
procedure ObserveContestMessages(
  const Values: TStrings;
  const Input: TJSONObject);
var
  Contests: TJSONArray;
  CqText: string;
  Index: Integer;
  TuAfterResetText: string;
  TuBeforeText: string;
  TuThresholdText: string;
begin
  RequireExactObjectFields(
    Input,
    [
  ...
  if RequireInteger(Input, 'stationIdRate', 0, MaxInt)
      <> ExpectedStationIdRate then
    raise Exception.Create('stationIdRate does not match the fixed vector');
  Contests := RequireContests(Input);
  ...
  Values.Add(
    ...
    + '|station-id-rate=' + IntToStr(ExpectedStationIdRate)
    + '|contest-count=' + IntToStr(Contests.Count));

  for Index := 0 to Contests.Count - 1 do
  begin
    CreateRuntime(Index);
    CqText := CaptureMessage(msgCQ);
    CompleteTransmission(CqText);

    TuBeforeText := CaptureMessage(msgTU);
    CompleteTransmission(TuBeforeText);

    Tst.OnSaveQsoComplete;
    Tst.OnSaveQsoComplete;
    TuThresholdText := CaptureMessage(msgTU);
    Tst.OnSaveQsoComplete;
    CompleteTransmission(TuThresholdText);

    TuAfterResetText := CaptureMessage(msgTU);
    CompleteTransmission(TuAfterResetText);

    RequireNoHandles;
    Values.Add(
      'contest[' + IntToStr(Index) + ']'
      + '|id=' + ExpectedContestIds[Index]
      + '|run-mode=' + ExpectedRunModeIds[Index]
      + '|cq=' + CqText
      + '|tu-before=' + TuBeforeText
      + '|tu-threshold=' + TuThresholdText
      + '|tu-after-reset=' + TuAfterResetText);
    DestroyRuntime;
  end;

  if Values.Count <> ExpectedContestCount + 1 then
    raise Exception.Create('contest-message scenario emitted invalid rows');
end;
...
begin
  ...
  try
    ...
    ObserveContestMessages(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
