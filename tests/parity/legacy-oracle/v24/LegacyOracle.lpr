...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v24';
  ExpectedSource =
    'tests/parity/legacy-oracle/v24/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v24/build-recipe.json';
  ExpectedScenario =
...
procedure ObserveHstRemoteExchangeFormat(
  const Values: TStrings;
  const Input: TJSONObject);
var
...
begin
  ...
  if RequireString(Input, 'contestId') <> 'scHst' then
    raise Exception.Create('contestId does not match the fixed vector');
  if RequireString(Input, 'runModeId') <> 'rmHst' then
    raise Exception.Create('runModeId does not match the fixed vector');
  ...
  if Values.Count <> 2 then
    raise Exception.Create('HST exchange scenario emitted invalid rows');
end;
...
begin
  ...
  try
    ...
    ObserveHstRemoteExchangeFormat(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
