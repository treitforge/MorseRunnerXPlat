...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v23';
  ExpectedSource =
    'tests/parity/legacy-oracle/v23/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v23/build-recipe.json';
  ExpectedScenario =
    'contest.naqp-remote-exchange-format-seed-12345';
  ExpectedStationCall = 'W7SST';
  ExpectedRemoteCall = 'K1ABC';
  ExpectedRst = 599;
  ExpectedExchange1 = 'DAVID';
  ExpectedExchange2 = 'CO';
  ExpectedSeed = 12345;
...
procedure ObserveNaqpRemoteExchangeFormat(
  const Values: TStrings;
  const Input: TJSONObject);
var
...
begin
  ...
  if RequireString(Input, 'contestId') <> 'scNaQp' then
    raise Exception.Create('contestId does not match the fixed vector');
  ...
  if Values.Count <> 2 then
    raise Exception.Create('NAQP exchange scenario emitted invalid rows');
end;
...
begin
  ...
  try
    ...
    ObserveNaqpRemoteExchangeFormat(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
