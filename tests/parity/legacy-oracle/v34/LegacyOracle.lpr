...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v34';
  ExpectedSource =
    'tests/parity/legacy-oracle/v34/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v34/build-recipe.json';
  ExpectedScenario =
    'contest.rare-rst-error-remote-exchange-format-seed-12345';
  ExpectedStationCall = 'W7SST';
  ExpectedRemoteCall = 'DL1ABC';
  ExpectedRst = 599;
  ExpectedExchange2 = 'ARRL';
  ExpectedFormatterCheckpointDraw = 5;
  ExpectedSeed = 12345;
...
procedure ObserveRareRstErrorContest(
  const Values: TStrings;
...
begin
  ...
  try
    for DrawIndex := 2 to ExpectedFormatterCheckpointDraw do
      Discarded := Random;
    if Discarded < 0 then
...
procedure ObserveRareRstErrorRemoteExchangeFormat(
  const Values: TStrings;
  const Input: TJSONObject);
begin
  RequireExactObjectFields(
  ...
  if Values.Count <> 2 then
    raise Exception.Create(
      'rare RST error exchange scenario emitted invalid rows');
end;
...
begin
  ...
  try
    ...
    ObserveRareRstErrorRemoteExchangeFormat(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
