...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v25';
  ExpectedSource =
    'tests/parity/legacy-oracle/v25/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v25/build-recipe.json';
  ExpectedScenario =
...
procedure ObserveWpxMidContestRemoteExchangeFormat(
  const Values: TStrings;
  const Input: TJSONObject);
var
...
begin
  ...
  if Values.Count <> 2 then
    raise Exception.Create('WPX exchange scenario emitted invalid rows');
end;
...
begin
  ...
  try
    ...
    ObserveWpxMidContestRemoteExchangeFormat(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
