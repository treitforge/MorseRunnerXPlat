...
{$ifndef V60_TUI_WPM_STEP_LOWER_CLAMP}
  {$fatal V60_TUI_WPM_STEP_LOWER_CLAMP must be defined}
{$endif}
...
const
  ...
  ExpectedVersionId = 'legacy-oracle-v60';
  ExpectedSource =
    'tests/parity/legacy-oracle/v60/LegacyOracle.lpr';
  ExpectedBuildRecipe =
    'tests/parity/legacy-oracle/v60/build-recipe.json';
  ExpectedScenario =
    'ux.tui-wpm-step-lower-clamp-page-up-from-zero-seed-12345';
...
procedure ObserveWpmStepLowerClamp(
  const Values: TStrings;
  const Input: TJSONObject);
var
...
begin
  ...
  if (Seed <> ExpectedSeed)
    or (PersistedStepWpm <> 0)
    or (EffectiveStepWpm <> 1)
    or (InitialWpm <> 30)
    or (ExpectedAfterWpm <> 31)
    or (RunModeId <> 'rmSingle') then
    raise Exception.Create(
      'WPM step lower-clamp input does not match its pinned contract');
  ...
  try
    ...
    try
      ...
      if Ini.WpmStepRate <> EffectiveStepWpm then
        raise Exception.Create(
          'CE persisted WPM step did not clamp to its lower bound');
    finally
      FreeAndNil(MainForm);
    end;
    ...
    if (Ini.Wpm <> InitialWpm)
      ...
      raise Exception.Create(
        'CE WPM step lower-clamp runtime did not start at its contract');
    Values.Add(
      'configuration'
      + '|run-mode=' + RunModeId
      + '|seed=' + IntToStr(Seed)
    ...
    if (Key <> 0) or (Ini.Wpm <> ExpectedAfterWpm) then
      raise Exception.Create(
        'CE lower-clamped WPM step handler reached an invalid value');
    Values.Add(
    ...
    if Values.Count <> 3 then
      raise Exception.Create(
        'WPM step lower-clamp capture emitted an invalid row count');
  finally
    DestroyRuntime;
...
begin
  ...
  try
    ...
    ObserveWpmStepLowerClamp(Values, Input);
    Emit(
      Scenario,
      AdapterId,
      VersionId,
