...
- Overlapping case surface/platform assignments: 63
...
- Active acceptance cases: 14
- Evidence-certified both-green cases: 0
- Legacy-green/XPlat-red cases: 14
...
| `simulation.runtime-routines` | Legacy contest, station, and operator routines | `not-authored` | 0 | 93 | 60 | `Contest.pas`<br>`Station.pas`<br>`DxOper.pas`<br>`DxStn.pas`<br>`StnColl.pas`<br>`MyStn.pas`<br>`QrmStn.pas`<br>`QrnStn.pas` |
...
| `simulation.legacy-effects` | Legacy QSB and random effects | `partial` | 9 | 15 | 3 | `Qsb.pas`<br>`RndFunc.pas` |
