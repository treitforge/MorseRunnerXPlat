...
public sealed class XPlatFlutterNoStationNoiseInvarianceTarget :
    IParityTarget
{
    ...
    internal const string FunctionalDivergenceCode =
        "audio-flutter-no-station-noise-invariance-mismatch";
    ...
    private static readonly ClientId ParityClient =
        new("flutter-no-station-noise-invariance-parity");
        ...
        CancellationToken cancellationToken)
    {
        ...
        CapturedAudioBlock[] clean = await CaptureAsync(
            ...
            cancellationToken);
        CapturedAudioBlock[] flutter = await CaptureAsync(
            input,
            flutterEnabled: true,
            cancellationToken);
        return Normalize(
        ...
        CancellationToken cancellationToken)
    {
            ...
            || snapshot.ActiveStations is not { Count: 0 })
        {
            throw new InvalidOperationException(
                "The XPlat flutter invariance session did not remain a "
    ...
    internal static string[] Normalize(
        ...
        IReadOnlyList<CapturedAudioBlock> flutter)
    {
        ArgumentNullException.ThrowIfNull(input);
        ...
        ValidateCapture(input, flutter, "flutter");
        ...
        AddBlockRows(values, "flutter", input, flutter);
             ...
             blockIndex++)
        {
            ...
            string flutterHash =
                ComputeRawSingleSha256(flutter[blockIndex].Samples);
            values.Add(
                $"output-invariance[{Format(blockIndex)}]"
                + $"|clean-float-sha256={cleanHash}"
                + $"|flutter-float-sha256={flutterHash}"
                + "|exact-equal="
                + Format(
                    SamplesEqual(
                        clean[blockIndex].Samples,
                        flutter[blockIndex].Samples)));
        }

        float[] cleanAggregate =
            CreateAggregate(input, clean);
        float[] flutterAggregate =
            CreateAggregate(input, flutter);
        values.Add(
            ...
            + ComputeRawSingleSha256(flutterAggregate)
            + "|exact-equal="
            + Format(SamplesEqual(cleanAggregate, flutterAggregate)));

        if (values.Count
            != FlutterNoStationNoiseInvarianceInput.ExpectedValueCount)
        {
            throw new InvalidDataException(
                "The flutter invariance capture emitted an invalid row "
                + "count.");
        }
        ...
        string run)
    {
        ...
        {
            throw new InvalidDataException(
                $"The {run} flutter invariance capture has an invalid "
                + "block count.");
        }
             ...
             blockIndex++)
        {
            ...
            {
                throw new InvalidDataException(
                    $"The {run} flutter invariance block {blockIndex} has "
                    + "invalid framing.");
            }
            ...
            foreach (float sample in block.Samples)
            {
                ...
                {
                    throw new InvalidDataException(
                        $"The {run} flutter invariance block {blockIndex} "
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException(
                $"The XPlat flutter invariance {action} command was "
