...
public sealed class XPlatOperatorMonitorRuntimeChangeTarget : IParityTarget
{
    ...
    internal const string FunctionalDivergenceCode =
        "audio-operator-monitor-runtime-change-mismatch";
    ...
    private static readonly ClientId ParityClient =
        new("operator-monitor-runtime-change-parity");
        ...
        CancellationToken cancellationToken)
    {
        ...
        OperatorMonitorRuntimeChangeInput input =
            OperatorMonitorRuntimeChangeInput.Parse(scenario);
        ...
        CancellationToken cancellationToken)
    {
            ...
            DurationBlocks: 0)
        {
            ...
            Flutter = false,
            Lids = false,
            MonitorLevelDb = input.FullMonitorLevelDb,
        };
    ...
    internal static string[] Normalize(
        ...
        CapturedRun runtimeMutedMonitor)
    {
        ArgumentNullException.ThrowIfNull(input);
        ArgumentNullException.ThrowIfNull(fullMonitor);
        ArgumentNullException.ThrowIfNull(runtimeMutedMonitor);
        ValidateRun(input, fullMonitor, "full-monitor");
        ValidateRun(input, runtimeMutedMonitor, "runtime-muted-monitor");

        int firstDivergence = FirstDivergence(
            fullMonitor.Blocks[1].Samples,
            runtimeMutedMonitor.Blocks[1].Samples);
        var values = new List<string>(
            OperatorMonitorRuntimeChangeInput.ExpectedValueCount)
        {
            "configuration"
            + "|run-mode=rmStop"
            ...
            + "-to-" + Format(input.RuntimeMonitorLevelDb)
        ...
        values.Add(
            ...
            + ComputeRawSingleSha256(fullMonitor.Blocks[1].Samples)
            + "|monitor-runtime-muted-float-sha256="
            + ComputeRawSingleSha256(runtimeMutedMonitor.Blocks[1].Samples));
        values.Add(
            "terminal-random"
        ...
        if (values.Count != OperatorMonitorRuntimeChangeInput.ExpectedValueCount)
        {
    ...
    private sealed class StrictCaptureAudioSink : IAudioSink
    {
        ...
        public CapturedAudioBlock[] RequireTwoBlocks() =>
            _blocks.Count == 2
                ? [.. _blocks]
                : throw new InvalidOperationException(
                    "The monitor parity sink did not capture two blocks.");
    }
}

internal sealed record OperatorMonitorRuntimeChangeInput(
    int SampleRate,
    int BlockSize,
    int Seed,
    ...
    IReadOnlyList<int> ProbeSampleIndexes)
{
    ...
    public static OperatorMonitorRuntimeChangeInput Parse(ParityScenario scenario)
    {
        ...
        int[] probes = input
            ...
            .ToArray();
        var result = new OperatorMonitorRuntimeChangeInput(
            ...
            input.GetProperty("runtimeMonitorLevelDb").GetInt32(),
        ...
        if (discriminator != XPlatOperatorMonitorRuntimeChangeTarget.ParityId
            ...
            || result.RuntimeMonitorLevelDb != -60
