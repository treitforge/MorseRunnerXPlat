...
public sealed class XPlatCwtRemoteExchangeFormatTarget : IParityTarget
{
    ...
    internal const string FunctionalDivergenceCode =
        "contest-cwt-remote-exchange-format-mismatch";
        ...
        CancellationToken cancellationToken)
    {
        ...
        CwtRemoteExchangeFormatInput input =
            CwtRemoteExchangeFormatInput.Parse(scenario);
    ...
    internal static string[] Observe(CwtRemoteExchangeFormatInput input)
    {
...
internal sealed record CwtRemoteExchangeFormatInput(
    string ContestId,
    string RunModeId,
    string StationCall,
    string RemoteCall,
    ...
    int Seed)
{
    public static CwtRemoteExchangeFormatInput Parse(ParityScenario scenario)
    {
        ...
        var result = new CwtRemoteExchangeFormatInput(
        ...
        string discriminator = RequireString(
            ...
            "scenario",
            scenario.Id);
        if (discriminator != XPlatCwtRemoteExchangeFormatTarget.ParityId
            || result.ContestId != "scCwt"
            || result.RunModeId != "rmPileup"
            || result.StationCall != "W7SST"
            || result.RemoteCall != "K1ABC"
            || result.Rst != 599
            || result.Exchange1 != "DAVID"
            || result.Exchange2 != "123"
            || result.Seed != 12345
