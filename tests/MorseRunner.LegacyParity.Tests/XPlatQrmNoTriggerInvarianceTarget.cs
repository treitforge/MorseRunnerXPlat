...
public sealed class XPlatQrmNoTriggerInvarianceTarget :
    IParityTarget
{
    ...
    private static readonly ClientId ParityClient =
        new("qrm-no-trigger-invariance-parity");
        ...
        CancellationToken cancellationToken)
    {
        ...
        int activeStationCount = snapshot.ActiveStations?.Count
            ?? throw new InvalidOperationException(
                "The XPlat QRM no-trigger snapshot omitted stations.");
        ...
        {
            throw new InvalidOperationException(
                "The XPlat QRM no-trigger session did not remain a "
        ...
        CapturedRun qrm)
    {
        ...
        AddBlockRows(values, "qrm", input, qrm.Blocks);
        values.Add(
            "station-counts"
            + $"|clean={Format(clean.ActiveStationCount)}"
            + $"|qrm={Format(qrm.ActiveStationCount)}");
        for (int blockIndex = 0;
             blockIndex < input.ComparedBlockCount;
             blockIndex++)
        {
            string cleanHash =
                ComputeRawSingleSha256(clean.Blocks[blockIndex].Samples);
            string qrmHash =
                ComputeRawSingleSha256(qrm.Blocks[blockIndex].Samples);
            values.Add(
                ...
                + Format(
                    SamplesEqual(
                        clean.Blocks[blockIndex].Samples,
                        qrm.Blocks[blockIndex].Samples)));
        }
            ...
            != QrmNoTriggerInvarianceInput.ExpectedValueCount)
        {
            throw new InvalidDataException(
                "The QRM no-trigger capture emitted an invalid row "
                + "count.");
        }
    ...
    private static void ValidateCapture(
        ...
        string run)
    {
        CapturedAudioBlock[] blocks = capture.Blocks;
        if (capture.ActiveStationCount != 0)
        {
            throw new InvalidDataException(
                $"The {run} QRM no-trigger capture created a station.");
        }

        if (blocks.Length != input.ComparedBlockCount)
        {
            throw new InvalidDataException(
                $"The {run} QRM no-trigger capture has an invalid "
                + "block count.");
        }
             ...
             blockIndex++)
        {
            ...
            {
                throw new InvalidDataException(
                    $"The {run} QRM no-trigger block {blockIndex} has "
                    + "invalid framing.");
            }
            ...
            foreach (float sample in block.Samples)
            {
                ...
                {
                    throw new InvalidDataException(
                        $"The {run} QRM no-trigger block {blockIndex} "
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException(
                $"The XPlat QRM no-trigger {action} command was "
