...
internal sealed record QrmNoTriggerInvarianceInput(
    int SampleRate,
    int BlockSize,
    int Seed,
    int BandwidthHz,
...
{
    internal const int ExpectedValueCount = 5;
    ...
    public static QrmNoTriggerInvarianceInput Parse(
        ParityScenario scenario)
    {
        ...
        {
            throw new InvalidDataException(
                $"Parity case '{scenario.Id}' QRM no-trigger input is "
                + "invalid.");
        }
