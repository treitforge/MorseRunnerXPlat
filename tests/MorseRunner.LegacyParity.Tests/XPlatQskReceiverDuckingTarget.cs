...
public sealed class XPlatQskReceiverDuckingTarget : IParityTarget
{
    ...
    private static readonly ClientId ParityClient =
        new("qsk-receiver-ducking-parity");
        ...
        CancellationToken cancellationToken)
    {
        ...
        QskReceiverDuckingInput input =
            QskReceiverDuckingInput.Parse(scenario);
        ...
        CancellationToken cancellationToken)
    {
        ...
        CapturedRun qskOff = await CaptureAsync(
            input,
            qskEnabled: false,
            cancellationToken);
        CapturedRun qskOn = await CaptureAsync(
            input,
            qskEnabled: true,
            cancellationToken);
        return Normalize(input, qskOff, qskOn);
    }
        ...
        CancellationToken cancellationToken)
    {
            ...
            || snapshot.LastOperatorMessage != input.MessageText)
        {
            throw new InvalidOperationException(
                "The XPlat QSK session left its fixed first-CQ boundary.");
        }
    ...
    {
        ArgumentNullException.ThrowIfNull(input);
        ArgumentNullException.ThrowIfNull(qskOff);
        ArgumentNullException.ThrowIfNull(qskOn);
        ValidateRun(input, qskOff, "QSK-off");
        ValidateRun(input, qskOn, "QSK-on");

        int firstDivergence = FirstDivergence(
            qskOff.Block.Samples,
            qskOn.Block.Samples);
        var values = new List<string>(
            QskReceiverDuckingInput.ExpectedValueCount)
        {
            "configuration"
            + "|run-mode=rmStop"
        ...
        values.Add(
            ...
            + "|qsk-off-float-sha256="
            + ComputeRawSingleSha256(qskOff.Block.Samples)
            + "|qsk-on-float-sha256="
            + ComputeRawSingleSha256(qskOn.Block.Samples));
        values.Add(
            "terminal-random"
            + "|ordinal=" + Format(input.TerminalRandomOrdinal)
            + "|qsk-off-single-bits="
            + SingleBits(qskOff.TerminalRandom)
            + "|qsk-on-single-bits="
            + SingleBits(qskOn.TerminalRandom));

        if (values.Count != QskReceiverDuckingInput.ExpectedValueCount)
        {
            throw new InvalidDataException(
                "The QSK capture emitted an invalid row count.");
        }
    ...
    private static int FirstDivergence(float[] first, float[] second)
    {
        ...
        {
            throw new InvalidDataException(
                "The QSK captures have different lengths.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new InvalidOperationException(
                $"The XPlat QSK {action} command was rejected: "
    ...
    private sealed class StrictCaptureAudioSink : IAudioSink
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                throw new InvalidOperationException(
                    "The QSK parity sink received invalid initialization.");
            }
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                throw new InvalidOperationException(
                    "The QSK parity sink received invalid audio.");
            }
        ...
        public void RequireInitializedWithoutAudio(SessionId sessionId)
        {
            ...
            {
                throw new InvalidOperationException(
                    "The QSK parity sink was not cleanly initialized.");
            }
        }
        ...
        public void RequireNoAudio(string phase)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"The QSK parity sink received audio {phase}.");
            }
        }
        ...
        public CapturedAudioBlock RequireSingleBlock() =>
            _block
            ?? throw new InvalidOperationException(
                "The QSK parity sink did not capture one block.");
    }
}

internal sealed record QskReceiverDuckingInput(
    int SampleRate,
    int BlockSize,
    int Seed,
    ...
    IReadOnlyList<int> ProbeSampleIndexes)
{
    ...
    public static QskReceiverDuckingInput Parse(ParityScenario scenario)
    {
        ...
        int[] probes = input
            ...
            .ToArray();
        var result = new QskReceiverDuckingInput(
        ...
        if (discriminator != XPlatQskReceiverDuckingTarget.ParityId
